...
pick_v6_address(struct iasubopt **addr, struct shared_network *shared_network,
		...
		//const struct data_string *client_id)
		const struct data_string *client_id, struct packet *packet)
{
	struct ipv6_pool *p;
	int i;
	int start_pool;
	...
	do {
		...
		if ((p->pool_type == D6O_IA_NA) &&
			...
			(create_lease6_2(p, addr, &attempts, client_id,
				   cur_time + 120, packet) == ISC_R_SUCCESS)) {
			/*
