...
#include "dhcpd.h"
// Added by Mason Yu. for Delegation
#include <rtk/sysconfig.h>
#include <config_rtk.h>

#ifdef CTCOM_DHCPV6_OPT17
...
#ifdef CTCOM_DHCPV6_OPT17
static int valid_ct_opt17(struct packet *packet, struct client_state *client)
{
	int rval = ISC_FALSE;
	char *ifname = client->name ? client->name : client->interface->name;
	int ifIndex = getIfIndexByName(ifname);
	MIB_CE_ATM_VC_T entry = {0};
	int i;
	unsigned char checked = 0;
	struct option_state *options = packet->options;
	struct option_cache *oc = lookup_option(&vsio_universe, options, VENDOR_CT_ENT_NUM);
	
	log_debug("[CT-COM e8] Checking CT-COM option 17 for %s...\n", ifname);

	if(getWanEntrybyindex(&entry, ifIndex) < 0)
	{
		log_debug("WARNNING: %s is not a WAN interface. Do not check CT-COM option 17.", ifname);
		return ISC_TRUE;
	}

	// Treat as valid reply if one of configuration is matched
	// Match type 2 (pure string) first 
	// because option cache will be destroyed after parsing sub_code + sub_data
	for(i = 0 ; i < 4 ; i++)
	{
		// match sub_code + sub_data
		if(entry.dhcpv6_opt17_enable[i] && entry.dhcpv6_opt17_type[i] == 2)
		{
			checked = 1;

			if(oc == NULL)
			{
				log_debug("Need to check but CT-COM option 17 is not found. Ignore this message.");
				return ISC_FALSE;
			}

			// match value only
			// do not use strcmp directly becuase data may not end with '\0'
			if(strlen(entry.dhcpv6_opt17_value[i]) == oc->data.len - 1
				&& memcmp(entry.dhcpv6_opt17_value[i], oc->data.data + 1, oc->data.len - 1) == 0)
			{
				log_debug("\"%s\" match found", entry.dhcpv6_opt17_value[i]);
				rval = ISC_TRUE;
				break;
			}
			else
				log_debug("%s(%d) != %s(%d)", entry.dhcpv6_opt17_value[i], strlen(entry.dhcpv6_opt17_value[i]), oc->data.data + 1, oc->data.len - 1);

		}
	}

	// Treat as valid reply if one of configuration is matched
	for(i = 0 ; i < 4 && rval == ISC_FALSE ; i++)
	{
		// match sub_code + sub_data
		if(entry.dhcpv6_opt17_enable[i] && entry.dhcpv6_opt17_type[i] == 1)
		{
			checked = 1;

			if(oc == NULL)
			{
				log_debug("Need to check but CT-COM option 17 is not found\n");
				return ISC_FALSE;
			}

			// Parse option vsio.ctcom into encapsulation of ctcom.xxxx
			// skip first byte (total length).
			// This byte is not defined in RFC, but defined in spec. of e8 project.
			parse_option_buffer(options, oc->data.data + 1, oc->data.len - 1, &ctcom_universe);

			// look up the option we need to check
			oc = lookup_option(&ctcom_universe, options, entry.dhcpv6_opt17_sub_code[i]);

			if(oc)
			{
				if(oc->option->format[0] == 't')
				{
					if(strlen(entry.dhcpv6_opt17_sub_data[i]) == oc->data.len
						&& memcmp(entry.dhcpv6_opt17_sub_data[i], oc->data.data, oc->data.len) == 0)
					{
						rval = ISC_TRUE;
						break;
					}
				}
				else if(memcmp(entry.dhcpv6_opt17_sub_data[i], oc->data.data, oc->data.len) == 0)
				{
					// We may need to find a safer way to match.
					rval = ISC_TRUE;
					break;
				}
			}
			else
				log_debug("No sub_code %d in packet\n", entry.dhcpv6_opt17_sub_code[i]);
		}
	}

	if(!checked)
	{
		//no needs to check because all configurations are disabled,
		//treat as valid reply
		rval = ISC_TRUE;	
	}

	if(rval== ISC_FALSE)
		log_debug("No CT-COM option 17 match found.\n");
...
#ifdef CTCOM_DHCPV6_OPT17
	// Only check advertise and reply message.
	if(packet->dhcpv6_msg_type == DHCPV6_ADVERTISE 
		|| packet->dhcpv6_msg_type == DHCPV6_REPLY)
		rval = valid_ct_opt17(packet, client);
#endif
...
start_bound(struct client_state *client)
{
	...
	// Added by Mason Yu. for Delegation
#ifdef CONFIG_IPV6
	cmd_stop_delegation(path_dhclient_db);
#endif
	write_client6_lease(client, lease, 0, 1);
#ifdef CONFIG_IPV6
#if defined(CONFIG_USER_RADVD)
	// Added by Mason Yu. for Delegation	
	cmd_delegation(path_dhclient_db);
#endif
#endif
