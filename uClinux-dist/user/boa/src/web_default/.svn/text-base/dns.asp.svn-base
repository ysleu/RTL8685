...
<title>DNS <% multilang("Configuration"); %></title>
...
{	
	...
	if(sji_checkip(document.dns.dns1.value) == false){
		...
		alert("IP address\"" + document.dns.dns1.value + "\"is invalid !");
		return;
	}
...
//	if (document.dns.dns2.value=="") {
	if (document.dns.dns2.value=="") {
		if (document.dns.dns3.value=="") {	// Jenny,  buglist B059, dns2 couldn't be empty if dns3 is not empty
			...
			alert("The second DNSv4 name server's IP is empty !");
	...
	if(sji_checkip(document.dns.dns2.value) == false){
		document.dns.dns2.focus();
		alert("IP address\"" + document.dns.dns2.value + "\"is invalid !");
		return;
	}
	...
	if(sji_checkip(document.dns.dns3.value) == false){
		...
		alert("IP address\"" + document.dns.dns3.value + "\"is invalid !");
		return;
	}
...
function saveClickIpv6()
{
	if (document.dns.dnsv61.value=="") {
		...
		alert("The first DNSv6 name server's IP is empty !");
		return false;
	}
	
	if(document.dns.dnsv61.value != "" ){
		if (! isUnicastIpv6Address( document.dns.dnsv61.value) ){
			alert("Invalid the first DNSv6 name server's IP !");
			document.dns.dnsv61.focus();
			return false;
		}
	}
	...
	if (document.dns.dnsv62.value=="") {
		if (document.dns.dnsv63.value=="")	// Jenny,  buglist B059, dns2 couldn't be empty if dns3 is not empty
			...
			alert("The second DNSv6 name server's IP is empty !");
	...
	if(document.dns.dnsv62.value != "" ){
		if (! isUnicastIpv6Address( document.dns.dnsv62.value) ){
			alert("Invalid the second DNSv6 name server's IP !");
			document.dns.dnsv62.focus();
			return false;
		}
	}
	...
	if(document.dns.dnsv63.value != "" ){
		if (! isUnicastIpv6Address( document.dns.dnsv63.value) ){
			alert("The third DNSv6 name server's IP is empty !");
...
<blockquote>
		<h2><font color="#0000FF">DNS <% multilang("Configuration"); %></font></h2>
		<DIV align="left" style="padding-left:20px; padding-top:5px">
			<form id="form" action=/boaform/admin/formDNS method=POST name="dns">
			<tr><font size=2>
				<% multilang("This page is used to configure the DNS Server IP addresses."); %>
			</tr>			
		    ...
		    <tr>
		      <td width="40%" colspan="3"><font size=2>
		        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="dnsAuto" name="dnsMode" <% checkWrite("dns0"); %> onClick="autoDNSclicked()"><% multilang("Attain DNS Automatically"); %>
		      </td>
		      <td></td>
		    </tr>
		    <tr>
		      <td width="40%" colspan="3"><font size=2>
		        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="dnsManual" name="dnsMode" <% checkWrite("dns1"); %> onClick="manualDNSclicked()"><% multilang("Set DNS Manually"); %>
		      </td>
