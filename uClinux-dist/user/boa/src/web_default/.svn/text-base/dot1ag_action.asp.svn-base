...
<title><% multilang("CFM (802.1ag) Action"); %></title>
...
function update_ui()
{
	with(document.dot1agAction)
	{
		if(processing == true)
		{
			document.getElementById('status').innerHTML = "<b><% multilang("Status"); %>:</b> <% multilang("Processing..."); %>";
			start.disabled = true;
		}
		else
		{
			document.getElementById('status').innerHTML = "<b><% multilang("Status"); %>:</b> <% multilang("Ready"); %>";
...
<body onLoad="on_init();">
<blockquote>
<h2><font color="#0000FF"><% multilang("CFM (802.1ag) Action"); %></font></h2>

<form action=/boaform/formDot1agAction method=POST name="dot1agAction">

<font size=2><b><% multilang("Please select one of following entry"); %>:</b></font>
...
<tr>
	<td width=30%><font size=2><% multilang("Action"); %>:&nbsp;&nbsp;</font></td>
	<td><input type="radio" name="action" value="lbm" onClick="update_ui()"><font size=2> <% multilang("Send LBM"); %></font></td>
	<td><input type="radio" name="action" value="ltm" onClick="update_ui()"><font size=2> <% multilang("Send LTM"); %></font></td>
</tr>
<tr id="tr_multicast">
	<td><font size=2><% multilang("Use Multicast"); %>:</td>
	<td><input type="checkbox" name="multicast" value=1 onClick="update_ui()"></td>
</tr>
<tr>
    <td width=30%><font size=2><% multilang("Destination MAC Address"); %>:&nbsp;&nbsp;</td>
    <td><input type="text" name="mac" size="20" maxlength="17"></td>
</tr>
<tr id="tr_count">
    <td width=30%><font size=2><% multilang("LBM Count"); %>:&nbsp;&nbsp;</td>
    <td><input type="text" name="count" size="20" maxlength="5"></td>
</tr>
</table>

<input type="button" value="<% multilang("Start"); %>" name="start" onClick="startClick()">&nbsp;&nbsp;

<p>
<hr size=1 noshade align=top>
<div id="status"></div>
<b><% multilang("Result"); %>:</b>
