...
<title>DHCP <% multilang("Settings"); %></title>
...
function showDhcpSvr(ip)
{
	...
	if (document.dhcpd.dhcpdenable[0].checked == true)
		...
		document.getElementById('displayDhcpSvr').innerHTML=
			...
			'<% multilang("This page is used to configure the DHCP Server IP Address for DHCP Relay."); %>'+
			'</font></tr>'+
			'<tr><td><hr size=1 noshade align=top></td></tr>'+
			'</table>'+
			'<table border=0 width="500" cellspacing=4 cellpadding=0>'+
			'<tr>'+
			'<td width="40%"><font size=2><b>DHCP <% multilang("Server"); %> <% multilang("IP Address"); %>:</b></font></td>'+
		...
		html=
			'<table border=0 width="500" cellspacing=4 cellpadding=0>'+
			'<tr><font color="#000000" size=2>'+
			'<% multilang("Enable the DHCP Server if you are using this device as a DHCP server. This page lists the IP address pools available to hosts on your LAN. The device distributes numbers in the pool to hosts on your network as they request Internet access."); %>'+


			'</font></tr>'+
			'<tr><td><hr size=1 noshade align=top></td></tr>'+
			'<tr><td><font color="#000000" size=2><b><% multilang("LAN"); %> <% multilang("IP Address"); %>: </b><% getInfo("dhcplan-ip"); %>&nbsp;&nbsp;&nbsp;'+
			'<b><% multilang("Subnet Mask"); %>: </b><% getInfo("dhcplan-subnet"); %>'+
			'</font></td></tr>'+
			'</table>'+
			'<table border=0 width="500" cellspacing=4 cellpadding=0>'+
			'<tr>'+
		...
		html+=
			'<input type="button" value="<% multilang("Show Client"); %>" name="dhcpClientTbl" onClick="dhcpTblClick(\'/dhcptbl.asp\')" >'+
			'</td>'+
			'</tr>';
		...
		html += '<tr>'+
			'<td width="30%"><font size=2><b><% multilang("Max Lease Time"); %>:</b></td>'+
			'<td width="70%"><font size=2>'+
			'<input type="text" name="ltime" size=10 maxlength=9 value="<% getInfo("lan-dhcpLTime"); %>"><b> <% multilang("seconds"); %></b><b> (<% multilang("-1 indicates an infinite lease"); %>)</b>'+
			'</td>'+
			'</tr>'+
			'<tr>'+
			'<td width="30%"><font size=2><b><% multilang("Domain"); %><% multilang(" Name"); %>:</b></td>'+
			'<td width="70%">'+
			'<input type="text" name="dname" size=32 maxlength=29 value="<% getInfo("lan-dhcpDName"); %>">'+
			'</td>'+
			'</tr>'+
			'<tr>'+
			'<td width="30%"><font size=2><b><% multilang("Gateway Address"); %>:</b></td>'+
		...
		html +=
			...
			'<input type="button" value="<% multilang("Port-Based Filter"); %>" name="macIpTbl" onClick="macIpClick(\'/portBaseFilterDhcp.asp\')">'+
			'<input type="button" value="<% multilang("MAC-Based Assignment"); %>" name="macIpTbl" onClick="macIpClick(\'/macIptbl.asp\')">';
...
<h2><font color="#0000FF">DHCP <% multilang("Settings"); %></font></h2>
...
<table border=0 width="500" cellspacing=4 cellpadding=0>
  <tr><td><font color="#000000" size=2>
    <% multilang("This page is used to configure DHCP Server and DHCP Relay."); %>
  </font></td></tr>
  <tr><td><hr size=1 noshade align=top></td></tr>
...
<table border=0 width="500" cellspacing=4 cellpadding=0>
  <tr>
  <td><font size=2><b>DHCP <% multilang("Mode"); %>: </b>
  <% checkWrite("dhcpMode"); %>
