...
function showDhcpSvr(ip)
{
	...
	if (document.dhcpd.dhcpdenable[0].checked == true)
		...
		document.getElementById('displayDhcpSvr').innerHTML=
			...
			'<table border=0 width="500" cellspacing=4 cellpadding=0>'+
			'<tr>'+
			'<td width="40%"><font size=2><b>DHCP <% multilang("Server"); %> <% multilang("IP Address"); %>:</b></td>'+
		...
		html +=
			...
			'<input type="button" value="<% multilang("Device"); %> <% multilang("IP Range"); %>" name="clientIpTbl" onClick="clientIpClick(\'/devicetbl.asp\')">';
