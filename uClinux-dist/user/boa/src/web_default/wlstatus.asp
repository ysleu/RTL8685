...
<title><% multilang("WLAN Status"); %></title>
...
<h2><b><font color="#0000FF"><% multilang("WLAN Status"); %></font></b></h2>

<table border=0 width="500" cellspacing=0 cellpadding=0>
<tr><td><font size=2>
 <% multilang("This page shows the WLAN current status."); %>
</font></td></tr>
...
<table width=400 border=0>
  <script>
	...
	document.write('\
	<tr bgcolor="#EEEEEE">\
	<td width=100% colspan="2" bgcolor="#008000"><font color="#FFFFFF" size=2><b><% multilang("WLAN"); %> \
	<% multilang("Configuration"); %></b></font></td>\
	</tr>\
	<tr bgcolor="#EEEEEE">\
	<td width=40%><font size=2><b><% multilang("Mode"); %></b></td>\
...
else
{
	...
	document.write('</td>\
		<tr bgcolor="#DDDDDD">\
	    	<td width=40%><font size=2><b><% multilang("Band"); %></b></td>\
	    	<td width=60%><font size=2>');
	if (band[i] == 1)
	...
	<tr bgcolor="#DDDDDD">\
	<td width=40%><font size=2><b><% multilang("Channel Number"); %></b></td>\
	<td width=60%><font size=2>'+channel_drv[i] +'</td>\
	</tr>\
	<tr bgcolor="#EEEEEE">\
	<td width=40%><font size=2><b><% multilang("Encryption"); %></b></td>\
	<td width=60%><font size=2>');
	...
	if (wlanMode[i]!=2) {	//2 means WDS mode
		...
		if (wlanMode[i]==0 || wlanMode[i]==3 || wlanMode[i]==4) {
			document.write("<td width=40%%><font size=2><b><% multilang("Associated Clients"); %></b></td>\n");
			document.write("<td width=60%%><font size=2>"+clientnum[i]+"</td></tr>");
		}
		else {
			document.write("<td width=40%%><font size=2><b><% multilang("State"); %></b></td>\n");
	...
	if (!wlanDisabled[i] && (wlanMode[i]==0 || wlanMode[i]==3 )) {
		for (idx=0; idx<mssid_num; idx++) {
			if (!mssid_disable[idx]) {
				...
				<b><% multilang("Virtual AP"); %>'+(idx+1)+' <% multilang("Configuration"); %></b></font></td>\
				</tr>\
				<tr bgcolor="#EEEEEE">\
	    			<td width=40%><font size=2><b><% multilang("Band"); %></b></td>\
			    	<td width=60%><font size=2>');
				...
				document.write('</tr>\
				<tr bgcolor="#EEEEEE">\
				<td width=40%%><font size=2><b><% multilang("Associated Clients"); %></b></td>\
    ...
    if (rp_enabled[i])	// start of repeater
    {
    	...
    	<tr bgcolor="#EEEEEE">\
    	<td width=100% colspan="2" bgcolor="#008000"><font color="#FFFFFF" size=2><b><% multilang("WLAN"); %> \
    	<% multilang("Repeater Interface"); %><% multilang(" Configuration"); %></b></font></td>\
    	</tr>\
    	<tr bgcolor="#EEEEEE">\
    	<td width=40%><font size=2><b><% multilang("Mode"); %></b></td>\
    	<td width=60%><font size=2>' );
