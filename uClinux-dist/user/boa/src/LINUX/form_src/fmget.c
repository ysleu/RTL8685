...
//#ifdef CONFIG_USER_UPNPD
	...
	{"hidden_ssid",INFO_MIB,MIB_WLAN_HIDDEN_SSID},
	...
	{"wpa_cipher",INFO_MIB,MIB_WLAN_WPA_CIPHER_SUITE},
	{"wpa2_cipher",INFO_MIB,MIB_WLAN_WPA2_CIPHER_SUITE},
...
/*ping_zhang:20090930 END*/
	...
	{"gpon_loid", INFO_MIB, MIB_GPON_LOID},
#endif
#ifdef CONFIG_EPON_FEATURE	
	{"epon_loid", INFO_MIB, MIB_EPON_LOID},
#endif
...
static int fnget_wpsKey(int eid, request * wp, int argc, char **argv, char *buffer) {
	...
	if (vChar == WSC_ENCRYPT_WEP) {
		...
		mib_get(MIB_WLAN_WEP_DEFAULT_KEY, (void *)&key);
		if (vChar == 1) {
			if (key == 0)
...
int checkWrite(int eid, request * wp, int argc, char **argv)
{
	...
	{
		#ifdef CONFIG_VDSL
		boaWrite (wp, "%s", multilang("DSL Settings") );
		#else
		boaWrite (wp, "%s", multilang("ADSL Settings") );
		#endif /*CONFIG_VDSL*/
		return 0;
	}
	...
	if( !strncmp (name, "adsl_", 5) || !strncmp (name, "adsl_slv_", 9) )
	{
		...
		if (!strcmp (&name[ofs], "diag_title"))
		{
			...
			{
				#ifdef CONFIG_VDSL
				boaWrite (wp, "%s", multilang("DSL Slave Tone Diagnostics") );
				#else
				boaWrite (wp, "%s", multilang("ADSL Slave Tone Diagnostics") );
				#endif /*CONFIG_VDSL*/
			}else
#endif /*CONFIG_USER_XDSL_SLAVE*/
			{
				#ifdef CONFIG_VDSL
				boaWrite (wp, "%s", multilang("DSL Tone Diagnostics") );
				#else
				boaWrite (wp, "%s", multilang("ADSL Tone Diagnostics") );
				#endif /*CONFIG_VDSL*/
			}
			return 0;
		}
		if (!strcmp (&name[ofs], "diag_cmt"))
		{
			...
			{
				#ifdef CONFIG_VDSL
				boaWrite (wp, "%s", multilang("DSL Slave Tone Diagnostics. Only ADSL2/ADSL2+/VDSL2 support this function.") );
				#else
				boaWrite (wp, "%s", multilang("ADSL Slave Tone Diagnostics. Only ADSL2/2+ support this function.") );
				#endif /*CONFIG_VDSL*/
			}else
#endif /*CONFIG_USER_XDSL_SLAVE*/
			{
				#ifdef CONFIG_VDSL
				boaWrite (wp, "%s", multilang("DSL Tone Diagnostics. Only ADSL2/ADSL2+/VDSL2 support this function.") );
				#else
				boaWrite (wp, "%s", multilang("ADSL Tone Diagnostics. Only ADSL2/2+ support this function.") );
				#endif /*CONFIG_VDSL*/
			}
			return 0;
		}
		...
		if (!strcmp (&name[ofs], "title"))
		{
			...
			{
				#ifdef CONFIG_VDSL
				boaWrite (wp, "%s", multilang("DSL Slave Statistics") );
				#else
				boaWrite (wp, "%s", multilang("ADSL Slave Statistics") );
				#endif /*CONFIG_VDSL*/
			}else
#endif /*CONFIG_USER_XDSL_SLAVE*/
			{
				#ifdef CONFIG_VDSL
				boaWrite (wp, "%s", multilang("DSL Statistics") );
				#else
				boaWrite (wp, "%s", multilang("ADSL Statistics") );
				#endif /*CONFIG_VDSL*/
			}
			return 0;
		}
	...
	if ( !strcmp(name, "wdsEncrypt")) {
		int i;
		char* encryp[5] = {multilang("None"), "WEP 64bits", "WEP 128bits", "WPA (TKIP)", "WPA2 (AES)"};
	...
	if ( !strcmp(name, "wlctlband") ) {
		...
		if(vChar == PHYBAND_5G) {
			...
			boaWrite(wp, "<option value=\"1\">Auto</option>\n" );
		}
		else{
	...
	if ( !strcmp(name, "lan-dhcp-st") ) {
		...
		if (DHCP_LAN_SERVER == vChar)
			boaWrite(wp, multilang("Enabled"));
		else
		#endif
			boaWrite(wp, multilang("Disabled"));
		return 0;
	}
...
//add by ramen for create new menu
	...
	else if(!strcmp(name,"6rdShow"))
	{
		...
		multilang("WAN Interface Binding"), (dnsBindPvcEnable) ? "checked" : "");
		return 0;
		}
#endif
#ifdef CONFIG_IPV6
		...
		multilang("WAN Interface Binding"), (dnsv6BindPvcEnable) ? "checked" : "");
		return 0;
		}
#endif
#endif
...
void write_wladvanced(int eid, request * wp, int argc, char **argv)        //add by yq_zhou 1.20
{
	...
	boaWrite(wp,
     ...
     "<input type=\"radio\" name=\"preamble\" value=\"short\">%s</td></tr>\n",
		multilang("Preamble Type"), multilang("Long Preamble"),
		multilang("Short Preamble"));
	...
	if (wpsUseVersion) {
			...
			multilang("Broadcast"), multilang("SSID"), multilang("Enabled"), multilang("Disabled"));
	}
	else
#endif
	{
	...
	multilang("Relay Blocking"), multilang("Enabled"), multilang("Disabled"));
	...
	multilang("Protection"), multilang("Enabled"), multilang("Disabled"));
  	boaWrite(wp,
     "<tr><td width=\"30%%\"><font size=2><b>%s:</b></td>\n"
     "<td width=\"70%%\"><font size=2>\n"
     "<input type=\"radio\" name=\"aggregation\" value=\"enable\">%s&nbsp;&nbsp;\n"
     "<input type=\"radio\" name=\"aggregation\" value=\"disable\">%s</td></tr>\n",
	multilang("Aggregation"), multilang("Enabled"), multilang("Disabled"));
       boaWrite(wp,
	...
	multilang("Short GI"), multilang("Enabled"), multilang("Disabled"));
#endif /*WLAN_SUPPORT*/
}
...
void ShowWanMode(int eid, request * wp, int argc, char **argv)
{
...
#ifdef WLAN_WISP
	{
		...
		if(mode == CLIENT_MODE)
			boaWrite(wp, "");
#ifdef WLAN_UNIVERSAL_REPEATER
		else if(mode != WDS_MODE && rptEnabled)
			boaWrite(wp, "");
#endif		
		else
			boaWrite(wp, "disabled");
