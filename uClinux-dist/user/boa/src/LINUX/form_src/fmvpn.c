...
void formPPtP(request * wp, char *path, char *query)
{
	...
	if (strAddPptpClient[0]) {
		...
		for (i=0; i<pptpEntryNum; i++)
		{
			...
			{
				strcpy(tmpBuf, "pptp vpn interface has already exist!");
				goto setErr_pptp;
			}
...
// Mason Yu. Add VPN ifIndex
		...
		if (strVal[0]) {
			...
			for (i=0; i<pptpEntryNum; i++)
			{
				...
				if (tmpEntry.dgw)
				{
					strcpy(tmpBuf, "default gw has already exist!");
					goto setErr_pptp;
				}
			}
			...
			for (i=0; i<l2tpEntryNum; i++)
			{
				...
				if (l2tpEntry.dgw)
				{
					strcpy(tmpBuf, "default gw has already exist for L2TP VPN!");
					goto setErr_pptp;
				}
			}
...
#ifdef CONFIG_NET_IPIP
			{
				...
				for (i=0; i<ipEntryNum; i++) {
					...
					if (ipEntry.dgw)
					{
						strcpy(tmpBuf, "default gw has already exist for IPIP VPN!");
						goto setErr_pptp;
					}
				}
			}
...
// Mason Yu. Add VPN ifIndex
	...
	if (strAddPptpAccount[0]) {
		...
		for (i=0; i<accountNum; i++)
		{
			...
			{
				strcpy(tmpBuf, "pptp vpn account has already exist!");
				goto setErr_pptp;
			}
...
void formL2TP(request * wp, char *path, char *query)
{
...
#endif
	{
		...
		for (i=0; i<l2tpEntryNum; i++)
		{
			...
			{
				strcpy(tmpBuf, "l2tp vpn interface has already exist!");
				goto setErr_l2tp;
			}
...
// Mason Yu. Add VPN ifIndex
		...
		if (entry.dgw == 1) {
			...
			for (i=0; i<pptpEntryNum; i++)
			{
				...
				if (ptpEntry.dgw)
				{
					strcpy(tmpBuf, "default gw has already exist for PPTP VPN!");
					goto setErr_l2tp;
				}
			}
	...
	if (strAddL2tpAccount[0]) {
		...
		for (i=0; i<accountNum; i++)
		{
			...
			if (!strcmp(tmpAccount.name, account.name))
			{
				strcpy(tmpBuf, "l2tp vpn account has already exist!");
				goto setErr_l2tp;
			}
	...
	for (i=0; i<l2tpEntryNum; i++) {
		...
		if ( strVal[0] ) {
			if (tmpEntry.conntype != MANUAL) {
				applyL2TP(&tmpEntry, 0);		// delete ppp interface
				if ( !strcmp(strVal, "Connect") )
					applyL2TP(&tmpEntry, 1);	// add	a ppp interface
			}
			else if (tmpEntry.conntype == MANUAL) {
				if ( !strcmp(strVal, "Disconnect") )
					applyL2TP(&tmpEntry, 3);	// disconnect(down) for dial on demand
				if ( !strcmp(strVal, "Connect") ) {
					applyL2TP(&tmpEntry, 0);    // delete
					applyL2TP(&tmpEntry, 1);   	// new
					applyL2TP(&tmpEntry, 2);   	// connect(up) for dial on demand
				}
			}
		}
	}
...
#endif
				Entry.server,
				multilang(Entry.tunnel_auth ? "Challenge" : "None"),
				ppp_auth[Entry.authtype], Entry.mtu,
				multilang(Entry.dgw ? "On" : "Off"), state[l2tp_phase]);
		}
		else {
			nBytesSent += boaWrite(wp, "<tr>"
				"<td align=center width=\"3%%\" bgcolor=\"#C0C0C0\"><font size=\"2\"><input type=\"checkbox\" name=\"s%d\" value=1></td>\n"
				...
				multilang(Entry.tunnel_auth ? "Challenge" : "None"),
				ppp_auth[Entry.authtype], Entry.mtu,
				multilang(Entry.dgw ? "On" : "Off"),
				//state[l2tp_phase],
				ifname,
				//multilang((2 == l2tp_phase) ? "Disconnect" : "Connect"), ifname);
				multilang(action_str), ifname);
		}
	}
	return nBytesSent;
}
	...
	if(addConf[0]){
		...
		if(Entry.negotiationType ==0){
			...
			if(strlen(strVal)>128){
				strcpy(tmpBuf, "psk length error, too long!"); //length error, too long!
				goto setErr_IPsec;
			}
	...
	for (i=0; i<entryNum; i++)
	{
		...
		if (ipipEnable) {
			nBytesSent += boaWrite(wp, "<tr>"
				...
				multilang(Entry.dgw ? "On" : "Off"), ifname,
				//IPIP_STATUS[Entry.idx]?"ON":"OFF",
				multilang(IPIP_STATUS[Entry.idx] ? "Disconnect" : "Connect"),
				ifname);
		}
		else {
			nBytesSent += boaWrite(wp, "<tr>"
				...
				"</tr>",
				Entry.idx, ifname, local, remote,
				multilang(Entry.dgw ? "On" : "Off"), multilang("Off"));
		}
