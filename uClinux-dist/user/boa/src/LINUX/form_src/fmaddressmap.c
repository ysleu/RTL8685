...
// Add
	...
	if (str[0]) {
		...
		if (mibtotal >= 16){
			strcpy(tmpBuf, "Maximun rules exceeded!");
				goto setErr_route;
		}
...
void formAddressMap(request * wp, char *path, char *query)
{
	...
	if (str[0]) {
		...
		str = boaGetVar(wp, "addressMapType", "");
		if (!str[0]) {
 			strcpy(tmpBuf, "Error! No address Map Type!");
		...
		if (mapType!=ADSMAP_NONE && mapType!=ADSMAP_ONE_TO_ONE && mapType!=ADSMAP_MANY_TO_ONE
			...
			strcpy(tmpBuf, "Invalid Address Map Type." );
			goto setErr_addressMap;
		}
		vChar = (char)mapType;
		...
		if (mib_set( MIB_ADDRESS_MAP_TYPE, (void *)&vChar) == 0) {
  			strcpy(tmpBuf, "Set MIB_ADDRESS_MAP_TYPE error.");
			goto setErr_addressMap;
		}
		...
		if ( str[0] ) {
			if ( !inet_aton(str, &lsip) ) {
				strcpy(tmpBuf, "Invalid Local Start IP address value!");
				goto setErr_addressMap;
			}

			if ( !mib_set(MIB_LOCAL_START_IP, (void *)&lsip)) {
	  			strcpy(tmpBuf, "Set  MIB_LOCAL_START_IP MIB error!");
				goto setErr_addressMap;
			}
		}
		...
		if ( str[0] ) {
			if ( !inet_aton(str, &leip) ) {
				strcpy(tmpBuf, "Invalid Local End IP address value!");
				goto setErr_addressMap;
			}

			if ( !mib_set(MIB_LOCAL_END_IP, (void *)&leip)) {
	  			strcpy(tmpBuf, "Set  MIB_LOCAL_END_IP MIB error!");
				goto setErr_addressMap;
			}
		}
		...
		if ( str[0] ) {
			if ( !inet_aton(str, &gsip) ) {
				strcpy(tmpBuf, "Invalid Global Start IP address value!");
				goto setErr_addressMap;
			}

			if ( !mib_set(MIB_GLOBAL_START_IP, (void *)&gsip)) {
	  			strcpy(tmpBuf, "Set  MIB_GLOBAL_START_IP MIB error!");
				goto setErr_addressMap;
			}
		}
		...
		if ( str[0] ) {
			if ( !inet_aton(str, &geip) ) {
				strcpy(tmpBuf, "Invalid Global End IP address value!");
				goto setErr_addressMap;
			}

			if ( !mib_set(MIB_GLOBAL_END_IP, (void *)&geip)) {
	  			strcpy(tmpBuf, "Set  MIB_GLOBAL_END_IP MIB error!");
				goto setErr_addressMap;
			}
		}
	}
