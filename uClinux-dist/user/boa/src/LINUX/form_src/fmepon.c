...
{	
	...
	{		
		...
		{	  		
			strcpy(tmpBuf, "get epon LOID Error");			
			goto setErr;		
		}		
		boaWrite(wp, "%s", loid);		
		return 0;	
        ...
        if(!strcmp(name, "fmepon_passwd"))
        {               
        	//printf("[%s:%d] name =%s\n",__func__,__LINE__,name);
            if(!mib_get(MIB_EPON_PASSWD,  (void *)passwd))
            {
            	strcpy(tmpBuf, "get epon password Error");
                goto setErr;
            }
...
{	
	...
	{		
		...
		if(!mib_set(MIB_EPON_LOID, strData_loid))		
		{			
			strcpy(tmpBuf, "Save EPON LOID Error");			
			goto setErr;		
		}	
	}

	strData_passwd = boaGetVar(wp, "fmepon_passwd", "");
       if ( strData_passwd[0] )
        {
               // printf("===>[%s:%d] fmepon_passwd=%s\n",__func__,__LINE__,strData_passwd);
                if(!mib_set(MIB_EPON_PASSWD, strData_passwd))
                {
                        strcpy(tmpBuf, "Save EPON Password Error");
                        goto setErr;
                }
        }
	...
	{	
		...
		{	
			...
			if(strlen(strData)!=17)   //xx:xx:xx:xx:xx:xx
			{
				...
				sprintf(tmpBuf, "Incorrect mac address %s for index %d! Please input for xx:xx:xx:xx:xx:xx",strData, index);
				goto setErr_filter;	
			}
...
int showepon_LLID_MAC(int eid, request * wp, int argc, char **argv)
{
	...
	"</font></tr>\n", multilang("index"), multilang("LLID"),multilang("status"),
	multilang("MAC"));
