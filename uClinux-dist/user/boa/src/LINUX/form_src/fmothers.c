...
void formOthers(request * wp, char *path, char *query)
{
	...
	if (str[0]) {
		...
		if ( !mib_set(MIB_ATM_VC_AUTOSEARCH, (void *)&flag)) {
			strcpy(tmpBuf, "Set VC auto-search error!");
			goto setErr_others;
		}
	}
	...
	if (str[0]) {
		...
		if ( !mib_set(MIB_IPPT_ITF, (void *)&ippt_itf)) {
			strcpy(tmpBuf, "Set IP passthrough interface index error!");
			goto setErr_others;
		}
	}
	...
	if (str[0]) {
		...
		if ( !mib_set(MIB_IPPT_LEASE, (void *)&ippt_lease)) {
			strcpy(tmpBuf, "Set IP passthrough lease time error!");
			goto setErr_others;
		}
	}
	...
	if ( !mib_set(MIB_IPPT_LANACC, (void *)&ippt_lanacc)) {
		strcpy(tmpBuf, "Set IP passthrough LAN access error!");
		goto setErr_others;
	}
#endif
	...
	if ( !mib_set(MIB_SPC_ENABLE, (void *)&flag)) {
		strcpy(tmpBuf, "Set Single PC flag error!");
		goto setErr_others;
	}
	...
	if (str[0]) {
		...
		if ( !mib_set(MIB_SPC_IPTYPE, (void *)&flag)) {
			strcpy(tmpBuf, "Set Single PC IP type error!");
			goto setErr_others;
		}
	}
