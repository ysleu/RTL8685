...
static int add_auto_pvc_search_pair(request * wp, int action)
{
	...
	if(entryNum > MAX_PVC_SEARCH_PAIRS ) {
		strcpy(tmpBuf, "Auto-PVC search table was full!");
		goto setErr_filter;
	}
	...
	*/
	...
	if (!strValue[0]) {
		strcpy(tmpBuf, "Enter VCI (0~65535)!");
		goto setErr_filter;
	}
	//printf("add_auto_pvc_search: strValue(VCI) = %s\n", strValue);
	sscanf(strValue,"%u",&VCI);
	if ( VCI>65535) {
		strcpy(tmpBuf, "Invalid VCI!");
		goto setErr_filter;
	}

	if(VCI ==0 && VPI == 0) {
		strcpy(tmpBuf, "Invalid VPI and VCI (0,0)!");
		goto setErr_filter;
	}
	...
	if(action) {
		...
		if(mib_chain_delete(MIB_AUTO_PVC_SEARCH_TBL, i) != 1){
			strcpy(tmpBuf, "Error! Delete chain record.");
			goto setErr_filter;
		}
	}
...
static int retrieveIPv6Record(request * wp, MIB_CE_ATM_VC_Tp pEntry)
{
	...
	if( pEntry->cmode == ADSL_6RD ) {  // 6rd

		pEntry->AddrMode |= 8;
		pEntry->IpProtocol = 0x3; 	   // IPv4/IPv6
	...
	if( pEntry->cmode == ADSL_DSLite ) {  // DS-Lite
		pEntry->AddrMode |= 4;
		pEntry->cmode = ACC_MER; // DS-Lite only for MER
		pEntry->IpProtocol = 0x2; 	// IPv6 only
	}
#endif
...
static void retrieveBrMode(request * wp, MIB_CE_ATM_VC_Tp pEntry)
{
	...
	{
		if(pEntry->cmode == ADSL_BR1483 || strcmp(str_br, "ON") == 0)
...
static int retrieveDHCPOpts(request * wp, MIB_CE_ATM_VC_Tp pEntry)
{
	...
	if(pEntry->enable_opt_61)
	{
		...
		if(pEntry->duid_type == 2)	/* enterprise number + identifier */
		{
			...
			{
				strcpy(tmpBuf, "Invalid Enterprise Number.");
				goto setErr_filter;
			}
...
{
	char tmpBuf[100];
	char *strValue, *strIp, *strMask, *strGW;
	unsigned int vUInt;
...
//#endif
	...
	if ((entry.applicationtype& X_CT_SRV_OTHER) && (entry.cmode != ADSL_BR1483))
		entry.enableIGMP = 1;
#endif
#else
{
	...
	else // Wan IP setting
	{
		...
		if (entry.cmode == ADSL_MER1483 && !isSameSubnet(strIp, strGW, strMask)) {
			strcpy(tmpBuf, "Invalid IP address! It should be located in the same subnet.");
			goto setErr_filter;
		}
#endif
#ifdef CONFIG_IPV6
	...
	unsigned char sid;
	strValue = boaGetVar(wp, "sid", "");
	sid = atoi(strValue);
	if(sid>127)
	{
		strcpy(tmpBuf, "Invalid GPON sid, valid value is 0~127.");
		goto setErr_filter;
...
//		"\n</script>\n</head>\n"),  fmName, fmName, fmName, fmName, fmName, fmName, fmName, fmName, fmName, fmName);
	...
	if( (pEntry->cmode==ADSL_MER1483) || (pEntry->cmode==ADSL_RT1483) )
	{
		boaWrite(wp,
			"\n\nfunction checkDigit(str)\n"
			"{\n"
...
void formWanMode(request * wp, char *path, char *query)
{
	...
	if(strSubmit[0])
	{
		...
		{
			strcpy(tmpBuf, "Internal error! Set WAN mode failed!");
			goto setErr_filter;
		}
...
void formEth(request * wp, char *path, char *query)
{
...
#if 0 //move to formWanMode
	/* Magician: ADSL/Eth WAN switch */
	strSubmit = boaGetVar(wp, "submitwan", "");
	...
	if(strSubmit[0])
	{
		...
		wanmode = atoi(strValue);
		wanmode = wanmode >> 3? 0: wanmode; // Valid value check. Only least 3 bits is valid.

		if(wanmode)
		{
			printf("WAN mode is set to ATM=%d, Ethernet=%d, PTM=%d.\n", wanmode&MODE_ATM?1:0, wanmode&MODE_Ethernet?1:0, wanmode&MODE_PTM?1:0);
			mib_set(MIB_WAN_MODE, (void *)&wanmode);
			SaveAndReboot(wp);
			return;
		}
	...
	/* End Magician: ADSL/Eth WAN switch */
#endif
...
#ifdef CONFIG_RTK_RG_INIT
		if(entry.cmode == ADSL_BR1483 && entry.vlan == 1)
		{
			...
			if(mbtd == 0)
			{
				...
				if(entry.vid != vlan_id)
				{
					strcpy(tmpBuf, "VLAN ID does not match LAN VLAN ID!");
					goto setErr_filter;
				}
			}
		}
	...
	if(wlanset){
		selected = getWanEntrybyMedia(&Entry, mType);
		if (selected < 0) {
	...
	//		entry.dgw = Entry.dgw;
			...
			if (entry.cmode != Entry.cmode)
			{
				...
				if(wlanset){
					...
					if(strValue[0]){
						//entry.ifIndex &= (~0xFF);
						//entry.ifIndex |= ETH_INDEX(strValue[0]);
						entry.ifIndex = TO_IFINDEX(MEDIA_WLAN, PPP_INDEX(entry.ifIndex), (strValue[0] - '0'));
						if((strValue[0] - '0') != ETH_INDEX(Entry.ifIndex))
							need_restart_wlan = 1;
					}
					else
						entry.ifIndex = TO_IFINDEX(MEDIA_WLAN, PPP_INDEX(entry.ifIndex), 0);
				}
				else{
#endif
#if defined(CONFIG_RTL_MULTI_ETH_WAN) || defined(CONFIG_PTMWAN)
	...
	//			entry.dgw = 1;
				...
				if(wlanset){
					strValue = boaGetVar(wp, "wispItf", "");
					if(strValue[0]){
						entry.ifIndex = (Entry.ifIndex & (~0xFF));
						entry.ifIndex |= (strValue[0] - '0');
						//entry.ifIndex = TO_IFINDEX(MEDIA_WLAN, PPP_INDEX(entry.ifIndex), ETH_INDEX((strValue[0] - '0')));
						if((strValue[0] - '0') != ETH_INDEX(Entry.ifIndex))
							need_restart_wlan = 1;
					}
					else
	...
	//add by ramen for DNS bind pvc
			...
			if(mib_chain_get(MIB_ATM_VC_TBL,selected,&dnsPvcEntry)&&(dnsPvcEntry.cmode!=ADSL_BR1483))
			{
				...
				for(tempi=0;tempi<3;tempi++)
				{
					...
					{
						if(entry.cmode==ADSL_BR1483) pvcifIdx=DUMMY_IFINDEX;
	...
	if(strSubmit[0])
	{
		...
		if( strValue[0] )
		{
			...
			if (intVal <  1 || intVal > 255 ){
				strcpy(tmpBuf, "Invalid Detect Mult (1~255)");
				goto setErr_ip;
			}
		...
		if( strValue[0] )
		{
			...
			{
				strcpy(tmpBuf, "Invalid Min Tx Interval");
				goto setErr_ip;
			}
		...
		if( strValue[0] )
		{
			...
			if( (uintVal==ULONG_MAX)&&(errno==ERANGE) )
			{
				strcpy(tmpBuf, "Invalid Min Rx Interval");
				goto setErr_ip;
			}
		...
		if( strValue[0] )
		{
			...
			if( (uintVal==ULONG_MAX)&&(errno==ERANGE) )
			{
				strcpy(tmpBuf, "Invalid Min Echo Rx Interval");
				goto setErr_ip;
			}
		...
		if( strValue[0] )
		{
			...
			if (intVal <  0 || intVal > 5 ){
				strcpy(tmpBuf, "Invalid Auth Type");
				goto setErr_ip;
			}
		...
		if( Entry.bfd_authtype!=BFD_AUTH_NONE )
		{
			...
			if( strValue[0] )
			{
				...
				if (intVal <  0 || intVal > 255 ){
					strcpy(tmpBuf, "Invalid Auth Key ID (0~255)");
					goto setErr_ip;
				}
			...
			if( strValue[0] )
			{
				...
				strcpy(tmpBuf, "Invalid Auth Key Length");
				...
				for( bfd_idx=0; bfd_idx<bfdstrlen; bfd_idx++ )
				{
					...
					if(!(	((strValue[bfd_idx]>='0')&&(strValue[bfd_idx]<='9')) ||
						...
						((strValue[bfd_idx]>='a')&&(strValue[bfd_idx]<='f')) ))
					{
						strcpy(tmpBuf, "Invalid Auth Key value (0-9,a-f,A-F)");
							goto setErr_ip;
					}
				}
				bfd_i=0;
		...
		if( strValue[0] )
		{
			...
			if (intVal <  0 || intVal > 63 ){
				strcpy(tmpBuf, "Invalid DSCP (0~63)");
				goto setErr_ip;
			}
			Entry.bfd_dscp = (unsigned char)intVal;
		}

		if(Entry.cmode==ADSL_MER1483)
		{
			...
			if( strValue[0] )
			{
				...
				if (intVal <  0 || intVal > 7 ){
					strcpy(tmpBuf, "Invalid Ethernet Priority (0~7)");
					goto setErr_ip;
				}
...
int atmVcList2(int eid, request * wp, int argc, char **argv)
{
	...
	multilang("Select"), multilang("Interface"), multilang("Mode"),
	multilang("Encapsulation"), multilang("IP Address"),
#ifdef DEFAULT_GATEWAY_V1
	multilang("Remote"),
#endif
	multilang("Subnet Mask"), multilang("User"), multilang(" Name"));

#ifdef DEFAULT_GATEWAY_V1
	nBytesSent += boaWrite(wp, "<td align=center width=\"3%%\" bgcolor=\"#808080\"><font size=2>%s</td>\n", multilang("Default Route"));
#endif
...
//				strStatus = (char *)IF_NA;
			...
			if(Entry.cmode==ADSL_PPPoE)
			{
				if(Entry.PPPoEProxyEnable)
					strcpy(pppoeProxy, multilang("Enabled"));
				else
					strcpy(pppoeProxy, multilang("Disabled"));
			}
			#endif
		}
		else
...
//				strStatus = (char *)IF_NA;
...
#ifdef CONFIG_IPV6
			" value=\"s%d\" onClick=\"postVC2(%s,%s,'%s','%s','%s',%d, %d, %d,'%s','%s','%s','%s',%d,%d,%d,%d,'%s','%s', '%s', %d, %d, %d"
			"%d, %d,'%s','%s','%s', %d, %d, %d, '%s',%d,'%s',%d)\"></td>\n",
#else
...
#ifdef CONFIG_IPV6
			Entry.enable,
			Entry.IpProtocol, Entry.AddrMode, Ipv6AddrStr, RemoteIpv6AddrStr, RemoteIpv6EndPointAddrStr, Entry.Ipv6AddrPrefixLen,  Entry.Ipv6Dhcp, Entry.Ipv6DhcpRequest,
		...
		"%d, %d,'%s','%s','%s', %d, %d, %d, '%s',%d,'%s',%d)\"></td>\n",
#else
		"'%s','%s','%s','%s',%d,%d,%d,'%s','%s', '%s', %d, %d, %d, %d)\"></td>\n",
#endif
...
#ifdef CONFIG_IPV6
		remoteIp, netmask, Entry.dgw, conn_status, Entry.enable, Entry.itfGroup,
		Entry.IpProtocol, Entry.AddrMode, Ipv6AddrStr, RemoteIpv6AddrStr, RemoteIpv6EndPointAddrStr, Entry.Ipv6AddrPrefixLen,  Entry.Ipv6Dhcp, Entry.Ipv6DhcpRequest,
        SixrdBRv4IP, Entry.SixrdIPv4MaskLen, Entry.SixrdPrefix, Entry.SixrdPrefixLen);
#else
		remoteIp, netmask, Entry.dgw, conn_status, Entry.enable, Entry.itfGroup);
		...
		if_display_name, mode, vpi, vci, aal5Encap, multilang(strNapt),
#ifdef CONFIG_USER_RTK_WAN_CTYPE
		ctype,
#endif
#ifdef CONFIG_IGMPPROXY_MULTIWAN
...
//#if defined(AUTO_PVC_SEARCH_PURE_OAMPING) || defined(AUTO_PVC_SEARCH_AUTOHUNT)
	...
	" ONCLICK=updatepvcState() >&nbsp;&nbsp;%s</b>"
     	"</td>", multilang("Enable Auto-PVC Search")
//#endif
...
//#if defined(AUTO_PVC_SEARCH_TR068_OAMPING)
	...
	boaWrite(wp,
   	...
   	"<input type=\"submit\" name=\"autopvcapply\" value= \"%s\" >"
		"</td>"
		"</tr>"
...
//#if defined(AUTO_PVC_SEARCH_PURE_OAMPING) || defined(AUTO_PVC_SEARCH_AUTOHUNT)
	...
	, multilang("Apply")
#ifdef AUTO_PVC_SEARCH_TABLE
	, multilang("Add"), multilang("Delete"), multilang("Current Auto-PVC Table")
#endif
	);
...
if(!wlanset)
	...
	if(wlanset)
...
#endif
			boaWrite(wp, _PTI _PTI _PTI _PTI _PTI, _PME(dgw), _PME(vlan), _PME(vid), _PME(vprio), _PME(enable));
#ifdef CONFIG_IPV6
		...
		boaWrite(wp, _PTS _PTS _PTI _PTI,
			_PME(pppUsername), _PME(pppPassword), _PME(pppCtype), _PME(pppIdleTime));
		boaWrite(wp, _PTI _PTS _PTS,
			_PME(pppAuth), _PME(pppACName), _PME(pppServiceName));
#ifdef CONFIG_IPV6
		boaWrite(wp, _PTI _PTS _PTS _PTS _PTI _PTI _PTI,
...
void ShowBridgeMode(int eid, request * wp, int argc, char **argv)
{
	...
	"</select></font></td></tr>"
	, multilang("Enable Bridge"), multilang("Bridge Mode"));
#endif //PPPOE_PASSTHROUGH
}
...
void ShowIGMPSetting(int eid, request * wp, int argc, char **argv)
{
	...
	boaWrite(wp, "<td><font size=2><b>%s: </b><input type=\"checkbox\" name=\"igmpEnabled\"\n"
			"size=\"2\" maxlength=\"2\" value=\"ON\"></td>", multilang("Enable IGMP"));
#endif
}
...
void ShowPPPIPSettings(int eid, request * wp, int argc, char **argv)
{
	...
	boaWrite(wp, "<table id=tbl_ppp border=0 width=800 cellspacing=4 cellpadding=0>\n"
		...
		multilang("PPP Settings"), multilang("User"), multilang(" Name"),
		MAX_PPP_NAME_LEN, multilang("Password"), MAX_NAME_LEN - 1,
		multilang("Type"), multilang("Idle Time (sec)")
	);
	...
	if (!onATM) {
		boaWrite(wp, "</tr>\n"
			...
			"		</select>\n"
			"	</td>\n",
			multilang("Authentication Method"));
		boaWrite(wp, "</tr>\n"
			"<tr><th></th>\n"
			"	<td><font size=2><b>%s:</b></td>\n"
			...
			multilang("AC-Name"), MAX_NAME_LEN, multilang("Service-Name"), MAX_NAME_LEN);
	}
#ifdef CONFIG_USER_PPPOE_PROXY
	boaWrite(wp, "</tr>\n"
		"<tr><th></th>\n"
		...
		multilang("Enable Pppoe Proxy"), multilang("Max Proxy User"));
#endif
	...
	boaWrite(wp, "</tr>\n</table>\n"
		...
		"		<input type=\"checkbox\" name=\"ipUnnum\" size=\"2\" maxlength=\"2\" value=\"ON\"  onClick=\"ipModeSelection()\"></td>\n"
#endif
		"</tr>\n", multilang("WAN IP Settings"), multilang("Type")
#ifdef DEFAULT_GATEWAY_V2
		, multilang("Unnumbered")
#endif
		, multilang("Local"), multilang("IP Address")
#ifdef DEFAULT_GATEWAY_V1
		, multilang("Remote"), multilang("IP Address")
#endif
	...
	if(!onATM)
	{
		...
		boaWrite(wp,
			...
			, multilang("DHCP Option Settings") );
		...
		boaWrite(wp,
			...
			"</tr>\n"
			, multilang("Enable DHCP Option 60"), multilang("Vendor ID"));
		...
		boaWrite(wp,
			...
			, multilang("Enable DHCP Option 61"), multilang("IAID"), multilang("DUID")
			, multilang("Link-Layer address plus time"), multilang("Enterprise Number and Identifier")
			, multilang("Enterprise Number"), multilang("Identifier")
			, multilang("Link-Layer address"));
		...
		boaWrite(wp,
			...
			"</tr>\n"
			, multilang("Enable DHCP Option 125"), multilang("Manufacturer OUI")
			, multilang("Product Class"), multilang("Model Name"), multilang("Serial Number")
			);
		boaWrite(wp, "</table>\n");
	}
#endif // DHCPC_SEND_OPT
...
void ShowDefaultGateway(int eid, request * wp, int argc, char **argv)
{
	...
	"<font size=2><b>&nbsp;&nbsp;%s:</b></td>\n</tr>\n",
	multilang("Obtain default gateway automatically"),
	multilang("Use the following default gateway"));
	...
	multilang("Use Remote WAN IP Address"),
	multilang("Use WAN Interface"));
...
void ShowIPV6Settings(int eid, request * wp, int argc, char **argv)
{
		...
		boaWrite(wp, "<table id=\"tbipv6wan\" border=0 width=\"800\" cellspacing=4 cellpadding=0>\n"
				...
				"	        </td>\n"
				"	</tr>\n"
				"</table>\n", multilang("WAN Setting"), multilang("Address Mode"));
		...
		boaWrite(wp, "<div id=secIPv6Div style=\"display:none\">\n"
				...
				"	</tr>\n"
				"</table>\n"
				"</div>\n", multilang("IPv6 Address"), multilang("Gateway"));
		...
		boaWrite(wp, "<br>\n"
				...
				"	</tr>\n"
				"</table>\n", multilang("Enable DHCPv6 Client"));
		...
		boaWrite(wp, "	<div  id=\"dhcp6c_block\"  style=\"display:none\">\n"
				...
				"	      </td>\n"
				"	  </tr>\n", multilang("Request Options"),
				multilang("Request Address"));
		boaWrite(wp, "	   <tr>\n"
				"	     <td width=\"150px\"><font size=2><b>&nbsp;</b></td>\n"
				"	      <td>\n"
...
void ShowDSLiteSetting(int eid, request * wp, int argc, char **argv)
{
		...
		boaWrite(wp, "<div id=DSLiteDiv style=\"display:none\">\n"
				...
				"	</tr>\n", multilang("DS-Lite WAN config"), multilang("Local IPv6 Address"));
		boaWrite(wp, "	<tr>\n"
				"		<td width=\"300px\"><font size=2>%s:</td>\n"
				...
				"	</tr>\n"
				"</table>\n"
				"</div>\n", multilang("Remote IPv6 End point Address"), multilang("Gateway IPv6 Address"));
#endif
}
...
static void ShowWlanPortCheck(request * wp)
{
	...
	boaWrite(wp, "<input type=checkbox name=chkpt>WLAN0</font></td>\n");
#ifdef WLAN_DUALBAND_CONCURRENT
	boaWrite(wp, "<td><font size=2><input type=checkbox name=chkpt>WLAN1</font></td></tr>\n");
#else
...
void ShowPortBaseFiltering(int eid, request * wp, int argc, char **argv)
{
	...
	boaWrite(wp, "<div id=div_pmap>\n<table id=tbl_pmap border=0 width=800 cellspacing=4 cellpadding=0>\n"
			...
			"</td><td>&nbsp;</td></tr>\n", multilang("Filter DHCP Discover packet"));
	boaWrite(wp, "<tr nowrap>");
	for (i=PMAP_ETH0_SW0; i<=PMAP_ETH0_SW3; i++) {
		if (i < SW_LAN_PORT_NUM) {
			if (!(i&0x1))
...
void Show6rdSetting(int eid, request * wp, int argc, char **argv)
{
		...
		boaWrite(wp, "	<tr>\n"
				...
				"	</tr>\n"
				"</table>\n"
				"</div>\n", multilang("6rd Prefix (EX:\"2001:db8::\")"), multilang("6rd Prefix length"));
#endif
}
...
void ShowWispWanItf(int eid, request * wp, int argc, char **argv)
{
	...
	for(idx = 0; idx<NUM_WLAN_INTERFACE; idx++){
		...
		if( (wlan_mode==AP_MODE || wlan_mode==AP_WDS_MODE) && rptEnabled)
			...
			boaWrite(wp, "	  <option value=\"%d\">wlan%d</option>\n", idx, idx);
