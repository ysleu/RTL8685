...
static void update_on_band_changed(MIB_CE_MBSSIB_T *pEntry, int idx, int cur_band)
{
	if(wl_isNband(cur_band)) {	//n mode is enabled
		if (idx == 0) { // for root
			char wpaCipher, wpa2Cipher, root_encrypt;
			
			/*
			 * andrew: new test plan require N mode to
			 * avoid using TKIP.
			 */
			mib_get(MIB_WLAN_ENCRYPT, (void *)&root_encrypt);
			if (root_encrypt != WIFI_SEC_WPA2_MIXED) {
				wpaCipher = WPA_CIPHER_AES;
				wpa2Cipher= WPA_CIPHER_AES;
				mib_set(MIB_WLAN_WPA_CIPHER_SUITE, (void *)&wpaCipher);
				mib_set(MIB_WLAN_WPA2_CIPHER_SUITE, (void *)&wpa2Cipher);
			}
		}
		else {
			/*
...
void formWlanSetup(request * wp, char *path, char *query)
{
	...
	int warn = 0;
#ifdef WLAN_WISP
	char need_reboot = 0, need_restartwan = 0;
	int wisp_wan_id;
	...
	if ( strVal[0] ) {
		...
		if (mode == CLIENT_MODE) {
			...
			if (encrypt == WIFI_SEC_WPA || encrypt == WIFI_SEC_WPA2) {
				...
				if (mib_set(MIB_WLAN_ENCRYPT, (void *)&vChar) == 0) {
					strcpy(tmpBuf, strSetWLANENCRYPTErr);
					goto setErr_wlan;
				}
...
#endif
#ifdef WLAN_WISP
		if(wlan_idx == wisp_wan_id){
			char prev_mode;
			mib_get( MIB_WLAN_MODE, (void *)&prev_mode);
			if( (prev_mode==CLIENT_MODE && (mode==AP_MODE || mode==AP_WDS_MODE))
				|| ((prev_mode==AP_MODE || prev_mode==AP_WDS_MODE) && mode==CLIENT_MODE))
			{
				char wan_mode;
		...
		if ( mib_set( MIB_WLAN_MODE, (void *)&mode) == 0) {
			strcpy(tmpBuf, strSetMIBWLANMODEErr);
			goto setErr_wlan;
		}
#ifdef WLAN_WISP
		if(wlan_idx == wisp_wan_id){
			...
			if(
#ifdef WLAN_UNIVERSAL_REPEATER
				!((mode == AP_MODE || mode == AP_WDS_MODE) && rptEnabled) &&
#endif
				mode != CLIENT_MODE){
					char wan_mode;
					mib_get(MIB_WAN_MODE, (void *)&wan_mode);
	...
	if ( strVal[0] ) {
		...
		update_on_band_changed(0, 0, mode);
	...
	if(mib_get(MIB_WIFI_REGDOMAIN_DEMO, (void *)&vChar)!=0 && vChar!=0)
	{
	    ...
	    if ( strVal[0] ) {
			...
			if ( mib_set( MIB_HW_REG_DOMAIN, (void *)&reg_num) == 0) {
				strcpy(tmpBuf, "Set regdomain error\n");
				goto setErr_wlan;
			}
		}
	}
	...
	if (encrypt == WIFI_SEC_NONE || encrypt == WIFI_SEC_WEP) {
		...
		if (encrypt == WIFI_SEC_WEP) {
			...
			if (0 == i && wpsUseVersion != 0) {
				vChar = 1;
				if ( mib_set(MIB_WSC_DISABLE, (void *)&vChar) == 0) {
...
//xl_yue: translocate to basic_setting  for ZTE531B BRIDGE
	...
	if ( strRate[0] ) {
		if ( strRate[0] == '0' ) { // auto
			...
			if ( mib_set(MIB_WLAN_RATE_ADAPTIVE_ENABLED, (void *)&vChar) == 0) {
				strcpy(tmpBuf, strSetAdapRateErr);
				goto setErr_advance;
			}
		}
		else  {
			...
			}
			{
				...
				if ( mib_set(MIB_WLAN_FIX_RATE, (void *)&uInt) == 0) {
					strcpy(tmpBuf, strSetFixdRateErr);
					goto setErr_advance;
				}
			}
			strRate = boaGetVar(wp, "basicrates", "");
	...
	if (strHiddenSSID[0]) {
		...
		if ( mib_set(MIB_WLAN_HIDDEN_SSID, (void *)&vChar) == 0) {
			strcpy(tmpBuf, strSetBrodSSIDErr);
			goto setErr_advance;
		}
		#ifdef WPS20
	...
	if (strBlock[0]) {
		...
		if ( mib_set(MIB_WLAN_BLOCK_RELAY, (void *)&vChar) == 0) {
			strcpy(tmpBuf, strSetRelayBlockErr);
			goto setErr_advance;
		}
	}
	...
	if (strBlock[0]) {
		...
		if ( mib_set(MIB_WLAN_QoS, (void *)&vChar) == 0) {
			strcpy(tmpBuf, strSetWlanWMMErr);
			goto setErr_advance;
		}
	}
#endif
...
int wlSiteSurveyTbl(int eid, request * wp, int argc, char **argv)
{
	...
	"<td align=center width=\"10%%\" bgcolor=\"#808080\"><font size=\"2\"><b>%s</b></font></td>\n"
      	...
      	"<td align=center width=\"10%%\" bgcolor=\"#808080\"><font size=\"2\"><b>%s</b></font></td>\n"
	"<td align=center width=\"10%%\" bgcolor=\"#808080\"><font size=\"2\"><b>%s</b></font></td>\n",
	multilang("Channel"), multilang("Type"), multilang("Encryption"), multilang("Signal"));
	if ( mode == CLIENT_MODE 
#ifdef WLAN_UNIVERSAL_REPEATER
		|| rpt_enabled
#endif
...
//modified by xl_yue
	...
	multilang(strSelect), multilang(strMACAddr), multilang(strWdsComment));
