...
void formQosRuleEdit(request * wp, char* path, char* query)
{
	...
	if(action[0])
	{
		...
		{//add new one
			...
			{
				ERR_MSG("AddQosRule:Wrong argument format!");
				return;
			}
			...
			if(index<0)
			{
				ERR_MSG("UpdateQosRule: Wrong argument format!");
				return;
			}
...
void formQosPolicy(request * wp, char *path, char *query)
{
	...
	if (action[0]) {
		...
		if(!mib_set(MIB_TOTAL_BANDWIDTH, (void*)&totalbandwidth))
		{
			ERR_MSG("formQosPolicy: Cannot set total bandwidth into mib setting!\n");
			return;
		}
	}
