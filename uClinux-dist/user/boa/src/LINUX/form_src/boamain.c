...
//#endif //CONFIG_USER_PPPOMODEM
	...
	boaASPDefine("ShowGPONSIDInfo", ShowGPONSIDInfo);
	boaASPDefine("fmGPONSIDSet", fmGPONSIDSet);
...
//#endif /*CONFIG_VDSL*/
	...
	boaFormDefine("formDNS", formDns);						// DNS Configuration Form
...
//xl_yue
	boaASPDefine("vportMenu", vportMenu);

// Mason Yu. combine_1p_4p_PortMapping
#ifdef ITF_GROUP
	boaFormDefine("formItfGroup", formItfGroup);	// Interface grouping Form
...
int portTrgList(request * wp)
{
	...
	multilang("Name"), multilang("IP Address"), multilang("Port to Open"),
	multilang("Port to Open"), multilang("Enable"), multilang("Action"));
...
void initGamingPage(request * wp)
{
	...
	else
	{
		...
		multilang("Port Triggering"), multilang(" Configuration"));
	...
	multilang("Name"), multilang("IP Address"), multilang("Port to Open"),
	multilang("Port to Open"), multilang("Enable"));
	...
	found ? (Entry.enable == 1 ? multilang("Checked") :"") : "",
	(strlen(idx)==0)?"-1":idx
	);
	...
	multilang("Add"), multilang("Modify"), multilang("Reset"));
	boaWrite(wp,
...
void ShowWmm(int eid, request * wp, int argc, char **argv)
{
	...
	boaWrite(wp,
		...
		multilang("WMM Support"), multilang("Enabled"), multilang("Disabled"));
#endif
}
...
void initEthWan(request * wp)
{
	...
	else if(Entry.cmode == ACC_PPPOE){//pppoe
