	...
	if(strData = boaGetVar(wp, "start_delete", ""))
	{
		if(fp = fopen(START_SCRIPT, "w"))
		{
			...
			if( va_cmd( "/bin/flatfsd", 1, 1, "-s" ) )
				printf( "[%d]:exec 'flatfsd -s' error!",__FILE__ );
#endif
			OK_MSG1("The initiating(start) script file was deleted!", "/init_script.asp")
		}
		else
			ERR_MSG("Failed to delete initiating(start) script file!");
	}
	else
	...
	if(strData = boaGetVar(wp, "start_delete", ""))
	{
		if(fp = fopen(END_SCRIPT, "w"))
		{
			...
			if( va_cmd( "/bin/flatfsd", 1, 1, "-s" ) )
				printf( "[%d]:exec 'flatfsd -s' error!",__FILE__ );
#endif
			OK_MSG1("The initiating(end) script file was deleted!", "/init_script.asp")
		}
		else
			ERR_MSG("Failed to delete initiating(end) script file!");
	}
	else
		ERR_MSG("Unexpected error!");
}
