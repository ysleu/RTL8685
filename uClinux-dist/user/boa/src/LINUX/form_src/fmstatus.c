		...
		if (find_ppp_from_conf(tp)) {
			...
			if ( strSubmitP[0] ) {
				if ((strcmp(strSubmitP, "Connect") == 0)) {
					...
					strcpy(tmpBuf, "Invalid PPP action!");
					goto setErr_filter;
				}
			}
		}
...
int wan3GTable(int eid, request * wp, int argc, char **argv)
{
	...
	nBytesSent += boaWrite(wp, "  <tr bgcolor=\"#808080\">\n"
	                              ...
	                              "  </tr>\n",
				multilang("Interface"), multilang("Protocol"), multilang("IP Address"),
				multilang("Gateway"), multilang("Status"));
		...
		if( mib_chain_get( MIB_WAN_3G_TBL, 0, (void*)p) && p->enable )
		{
			...
			if(!pppDod)
			{
				nBytesSent += boaWrite(wp, "<input type=\"submit\" value=\"%s\" name=\"submit%s\">\n",
							(pppConnectStatus==1) ?
							multilang("Disconnect") : multilang("Connect"), mppp_ifname);
			}
...
int wanL2TPTable(int eid, request * wp, int argc, char **argv)
{
	...
	nBytesSent += boaWrite(wp, "  <tr bgcolor=\"#808080\">\n"
	                              ...
	                              "    <td width=\"20%%\" align=center><font size=2><b>%s</b></font></td>\n"
	                              "  </tr>\n",
		multilang("Interface"), multilang("Protocol"), multilang("Local"),
		multilang("IP Address"), multilang("Remote"), multilang("IP Address"),
		multilang("Status"));
...
int wanConfList(int eid, request * wp, int argc, char **argv)
{
	...
	multilang("Interface"), multilang("VPI/VCI"), multilang("Encapsulation"),
	multilang("Protocol"), multilang("IP Address"), multilang("Gateway"),
	multilang("Status"));
	in_turn = 0;
	for (i=0; i<ifcount; i++) {
		if (in_turn == 0)
...
int DSLVer(int eid, request * wp, int argc, char **argv)
{
	...
	nBytesSent += boaWrite(wp, FM_DSL_VER, multilang("DSP Version") , s_ver);

#ifdef CONFIG_USER_XDSL_SLAVE
	getAdslSlvInfo(ADSL_GET_VERSION, s_ver, 64);
	nBytesSent += boaWrite(wp, FM_DSL_VER, multilang("DSP Slave Version"), s_ver);
