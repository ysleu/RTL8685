...
void formRadvdSetup(request * wp, char *path, char *query)
{
	...
	//if (str[0]) {
		if ( !mib_set(MIB_V6_MAXRTRADVINTERVAL, (void *)str)) {
			strcpy(tmpBuf, "Set MaxRtrAdvIntervalAct mib error!");
			goto setErr_radvd;
		}
	//}
	...
	//if (str[0]) {
		if ( !mib_set(MIB_V6_MINRTRADVINTERVAL, (void *)str)) {
			strcpy(tmpBuf, "Set MinRtrAdvIntervalAct mib error!");
			goto setErr_radvd;
		}
	//}
	...
	//if (str[0]) {
		if ( !mib_set(MIB_V6_ADVCURHOPLIMIT, (void *)str)) {
			strcpy(tmpBuf, "Set AdvCurHopLimitAct mib error!");
			goto setErr_radvd;
		}
	//}
	...
	//if (str[0]) {
		if ( !mib_set(MIB_V6_ADVDEFAULTLIFETIME, (void *)str)) {
			strcpy(tmpBuf, "Set AdvDefaultLifetime mib error!");
			goto setErr_radvd;
		}
	//}
	...
	//if (str[0]) {
		if ( !mib_set(MIB_V6_ADVREACHABLETIME, (void *)str)) {
			strcpy(tmpBuf, "Set AdvReachableTime mib error!");
			goto setErr_radvd;
		}
	//}
	...
	//if (str[0]) {
		if ( !mib_set(MIB_V6_ADVRETRANSTIMER, (void *)str)) {
			strcpy(tmpBuf, "Set AdvRetransTimer mib error!");
			goto setErr_radvd;
		}
	//}
	...
	//if (str[0]) {
		if ( !mib_set(MIB_V6_ADVLINKMTU, (void *)str)) {
			strcpy(tmpBuf, "Set AdvLinkMTU mib error!");
			goto setErr_radvd;
		}
	//}
	...
	if (str[0]) {
		if ( !mib_set(MIB_V6_ULAPREFIX, (void *)str)) {
			strcpy(tmpBuf, "Set ULA prefix mib error!");
			goto setErr_radvd;
		}
	}
	...
	if (str[0]) {
		if ( !mib_set(MIB_V6_ULAPREFIX_LEN, (void *)str)) {
			strcpy(tmpBuf, "Set ULA prefix len mib error!");
			goto setErr_radvd;
		}
	}
	...
	if (str[0]) {
		if ( !mib_set(MIB_V6_ULAPREFIX_VALID_TIME, (void *)str)) {
			strcpy(tmpBuf, "Set ULA prefix valid time mib error!");
			goto setErr_radvd;
		}
	}
	...
	if (str[0]) {
		if ( !mib_set(MIB_V6_ULAPREFIX_PREFER_TIME, (void *)str)) {
			strcpy(tmpBuf, "Set ULA prefix prefered time mib error!");
			goto setErr_radvd;
		}
	}
		...
		if (pStr[0] == '1') {
			...
			if (str[0]) {
				if ( !mib_set(MIB_V6_PREFIX_IP, (void *)str)) {
					strcpy(tmpBuf, "Set prefix_ip mib error!");
					goto setErr_radvd;
				}
			}
			...
			if (str[0]) {
				if ( !mib_set(MIB_V6_PREFIX_LEN, (void *)str)) {
					strcpy(tmpBuf, "Set prefix_len mib error!");
					goto setErr_radvd;
				}
			}
			...
			//if (str[0]) {
				if ( !mib_set(MIB_V6_VALIDLIFETIME, (void *)str)) {
					strcpy(tmpBuf, "Set AdvValidLifetime mib error!");
					goto setErr_radvd;
				}
			//}
			...
			//if (str[0]) {
				if ( !mib_set(MIB_V6_PREFERREDLIFETIME, (void *)str)) {
					strcpy(tmpBuf, "Set AdvPreferredLifetime mib error!");
					goto setErr_radvd;
				}
			//}
