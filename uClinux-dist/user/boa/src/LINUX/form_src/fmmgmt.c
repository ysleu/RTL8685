...
int accountList(int eid, request * wp, int argc, char **argv)
{
	...
	multilang("Select"), multilang("User"), multilang(" Name"), multilang("Privilege"));
...
//	usName, PRIV_USER, upasswd);
	...
	for (i=0; i<entryNum; i++) {
		...
		if (Entry.privilege == PRIV_ROOT)
			priv = multilang("Admin");
		else if (Entry.privilege == PRIV_ENG)
			priv = multilang("Support");
...
void formUpload(request * wp, char *path, char *query)
{
	...
	if (upgrade != 2) {	//signature Err
		strcpy(tmpBuf, FILEWITHWRONGSIG);
		goto fail;
	}
#endif
	...
	// write to flash
	{
		...
		cmd_upload(wp->post_file_name, startPos, nLen);
		...
		boaWrite(wp, "<html><head><style>\n"
			 ...
			 "function start() { h--; if (h >= 40) { frm.time.value = h; frm.textname.value='Firmware upgrading, Please wait ...'; id=setTimeout(\"start()\",1000); }\n"
			 "if (h >= 0 && h < 40) { frm.time.value = h; frm.textname.value='System restarting, Please wait ...'; id=setTimeout(\"start()\",1000); }\n"
...
#ifdef CONFIG_USER_XMLCONFIG
		printf("formSaveConfig(Reset): Reset to default configuration... \n");
...
int adslPSDMeasure(int eid, request * wp, int argc, char **argv) {
	...
	Value = multilang((psd_measure) ? "Disable" : "Enable");
...
int sysLogList(int eid, request * wp, int argc, char **argv)
{
	...
	if (dbg_enable==0) {
		...
		multilang("Date"), multilang("Time"), multilang("Facility"),
		multilang("Level"), multilang("Message"));
...
void formSysLog(request * wp, char *path, char *query)
{
	...
	if (str[0]) {
		...
		if ( mib_set(MIB_MAXLOGLEN, (void *)&len) == 0) {
			strcpy(tmpBuf, "Set max msg. length MIB error!");
			goto formSysLog_err;
		}
	}
	...
	if (strVal[0]) {
		...
		if (strVal[0]) {
			...
			if (mib_set(MIB_SYSLOG_LOG_LEVEL, (void *)&logcap) == 0) {
				strcpy(tmpBuf, "Set Syslog Log Level error!");
				goto formSysLog_err;
			}
		}
		...
		if (strVal[0]) {
			...
			if (mib_set(MIB_SYSLOG_DISPLAY_LEVEL, (void *)&logcap) == 0) {
				strcpy(tmpBuf, "Set Syslog Display Level error!");
				goto formSysLog_err;
			}
		}
	...
	if (strVal[0]) {
		...
		if ( !mib_set( MIB_LOG_SERVER_IP, (void *)&inIp)) {
			strcpy(tmpBuf, "Set LOG Server IP error!");
			goto formSysLog_err;
		}
 	}
	...
	if (strVal[0]) {
		if ( !mib_set(MIB_LOG_SERVER_NAME, (void *)strVal)) {
			strcpy(tmpBuf, "Set user name for LOG Server IP error!");
			goto formSysLog_err;
		}
	}
...
void formNtp(request * wp, char *path, char *query)
{
	...
	if(strVal[0]){
		...
		if (mib_set(MIB_DST_ENABLED, &dst_enabled) == 0) {
			strcpy(tmpBuf, "Set dst_enabled flag error!");
			goto setErr_end;
		}
		...
		if (tmpStr[0]) {
			...
			if (mib_set(MIB_NTP_TIMEZONE_DB_INDEX, &index) == 0) {
				strcpy(tmpBuf, "Set Index of Time Zone Database error!");
				goto setErr_end;
			}
