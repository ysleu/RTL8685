...
/*end for debug*/
	...
	if(strData[0])
	{
		...
		{
			strcpy(tmpBuf, "Get WAN interface failed!");
			goto setErr_tr069;
		}
		...
		if( tr069_itf != pre_tr069_itf )
		{
			...
			{
				strcpy(tmpBuf, "Set WAN interface failed!");
				goto setErr_tr069;
			}
			changeflag = 1;
		}
...
int ShowMNGCertTable(request * wp)
{
	...
	nBytesSent += boaWrite(wp, "\n"
		...
		"		%s:\n"
		"	</td></tr>\n"
		"\n", multilang("Certificate Management"));
	...
	nBytesSent += boaWrite(wp, "\n"
		...
		"      </td>\n"
		"  </tr>\n"
		"\n", multilang("Certificate Password"), buffer, multilang("Apply"), multilang("Undo"));
	...
	nBytesSent += boaWrite(wp, "\n"
		...
		"      </td>\n"
		"  </tr>\n"
		"\n", multilang("Certificate"), multilang("Choose File"), multilang("Upload"));
...
int ShowDataModels(request * wp)
{
	...
	nBytesSent += boaWrite(wp, "	  <td width=\"70%%\"><font size=2>\n");
	nBytesSent += boaWrite(wp, "	  <input type=\"radio\" name=use_tr181 value=0 %s >InternetGatewayDevice(TR-098)&nbsp;&nbsp;\n", isEnable==0?"checked":"" );
	nBytesSent += boaWrite(wp, "	  <input type=\"radio\" name=use_tr181 value=1 %s >Device(TR-181)\n", isEnable==1?"checked":"" );
	nBytesSent += boaWrite(wp, "	  </td>\n");
	nBytesSent += boaWrite(wp, "  </tr>\n");
