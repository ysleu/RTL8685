...
static int checkEntry(MIB_CE_DDNS_Tp pEntry, int type, char *pMsg)
{
	...
	for (i=0; i<num; i++) {
		...
		if (pEntry->ServicePort != tmpEntry.ServicePort)
			continue;
		// entry duplication
		strcpy(pMsg, "Entry already exists!");
		return -1;
	}
	return 0;
}
...
//	unsigned char ddns_if, if_num;
	...
	if (str[0]) {
		...
		if (intVal == 0) {
			...
			//return;
			strcpy(tmpBuf, "Error: Add chain(MIB_DDNS_TBL) record.");
			goto setErr_route;
		}
		else if (intVal == -1) {
...
//	"<td align=center width=\"20%%\" bgcolor=\"#808080\">Interface</td>"
	...
	multilang("Hostname"), multilang("User"), multilang(" Name"), multilang("Service"), multilang("Status"));
...
//		Entry.Enabled ? "Enable" : "Disable", Entry.hostname, Entry.username, Entry.provider, Entry.interface);
		multilang(Entry.Enabled ? "Enable" : "Disable"), Entry.hostname, Entry.username, Entry.provider, statusStr);
