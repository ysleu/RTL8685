...
void form3GConf(request * wp, char *path, char *query)
{
	...
	{
		//printf( "%s: error mib_chain_get(), line=%d\n", __FUNCTION__, __LINE__ );
		strcpy(tmpBuf, "internal error: mib_chain_get() failed!");
		goto setErr_3g;
	}
	...
	if ( strData[0] )
	{
		if( (strlen(strData)!=4) ||
		    ...
		    isdigit(strData[3])==0 )
		{
			strcpy(tmpBuf, "Invalid PIN Code value!");
			goto setErr_3g;
		}
	...
	if ( strData[0] )
	{
		...
		{
			strcpy(tmpBuf, "Invalid MTU value!");
			goto setErr_3g;
		}
	}
	...
	if ( strData[0] )
	{
		...
		{
			strcpy(tmpBuf, "Invalid backup timer value!");
			goto setErr_3g;
		}
	}
	...
	{
		//printf( "%s: error mib_chain_get(), line=%d\n", __FUNCTION__, __LINE__ );
		strcpy(tmpBuf, "internal error: mib_chain_update() failed!");
		goto setErr_3g;
	}
