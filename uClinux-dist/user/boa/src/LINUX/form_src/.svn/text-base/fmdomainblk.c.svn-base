...
void formDOMAINBLK(request * wp, char *path, char *query)
{
	...
	if (str[0]) {
		...
		if (str[0]) {
			...
			if ( !mib_set(MIB_DOMAINBLK_CAPABILITY, (void *)&domainblkcap)) {
				strcpy(tmpBuf, "Set Domain Blocking Capability error!");
				goto setErr_route;
			}
		}
 	}
		...
		if (str[0]) {
			for (i=0; i<totalEntry; i++) {
				...
				if ( !gstrcmp(str, tmpBuf) ) {
					...
					if(mib_chain_delete(MIB_DOMAIN_BLOCKING_TBL, idx) != 1) {
						strcpy(tmpBuf, "Delete Domain Blocking chain record error!");
						goto setErr_route;
					}
	...
	if (str[0]) {
		...
		if (intVal == 0) {
			...
			//return;
			strcpy(tmpBuf, "Error: Add Domain Blocking chain record.");
			goto setErr_route;
		}
		else if (intVal == -1) {
...
int showDOMAINBLKTable(int eid, request * wp, int argc, char **argv)
{
	...
	multilang("Select"), multilang("Domain"));
