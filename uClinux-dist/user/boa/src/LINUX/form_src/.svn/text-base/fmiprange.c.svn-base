...
#define GOOD_MSG(url) { \
	boaHeader(wp); \
	boaWrite(wp, "<body><blockquote><h5>Change setting successfully!<BR>" \
	...
	if (strVal[0]) {
		...
		if (strVal[0]) {
			...
			for (i=0; i<totalEntry; i++) {
				...
				if (!gstrcmp(strVal, entry.poolname)) {
					strcpy(tmpBuf, "device is already exist in table!");
					goto setErr_dhcpd;
				}
			}
			...
			strcpy(tmpBuf, "device name should not be NULL!");
			goto setErr_dhcpd;
		}
		...
		if (strVal[0]) {
			if ( !inet_aton(strVal, (struct in_addr *)dhcpSpEntry.startaddr) ) {
				strcpy(tmpBuf, "source IP is wrong!");
				goto setErr_dhcpd;
			}
		} else {
			strcpy(tmpBuf, "source IP cannot be empty");
			goto setErr_dhcpd;
		}
		...
		if (strVal[0]) {
			if ( !inet_aton(strVal, (struct in_addr *)dhcpSpEntry.endaddr) ) {
				strcpy(tmpBuf, "destination IP is wrong!");
				goto setErr_dhcpd;
			}
		} else {
			strcpy(tmpBuf, "destination IP cannot be empty");
			goto setErr_dhcpd;
		}
...
/*ping_zhang:20090526 START:Add gateway for each ip range*/
		...
		if (strVal[0]) {
			if ( !inet_aton(strVal, (struct in_addr *)dhcpSpEntry.iprouter) ) {
				strcpy(tmpBuf, "gateway IP is wrong!");
				goto setErr_dhcpd;
			}
		} else {
			strcpy(tmpBuf, "gateway IP cannot be empty");
			goto setErr_dhcpd;
		}
/*ping_zhang:20090526 END*/
		...
		if (strVal[0]) {
			...
			strcpy(tmpBuf, "option60 cannot be NULL!");
			goto setErr_dhcpd;
		}
...
/*ping_zhang:20090930 START:add for Telefonica new option 240*/			
			...
			if (strVal[0]) {
				...
				strcpy(tmpBuf, "optionStr cannot be NULL!");
				goto setErr_dhcpd;
			}
/*ping_zhang:20090930 END*/
		}
		...
		else
		{
			...
			if (strVal[0]) {
				...
				strcpy(tmpBuf, "optionCode error!");
				goto setErr_dhcpd;
			}
			...
			for (i=0; i<totalEntry; i++) {
				...
				if ( !gstrcmp(strVal, tmpBuf) ) {
					if( !mib_chain_get( MIB_DHCPS_SERVING_POOL_TBL, idx, (void*)&dhcpSpEntry ) ) {
						strcpy(tmpBuf, "Get chain(MIB_DHCPS_SERVING_POOL_TBL) record error!\n");
						goto setErr_dhcpd;
					}
...
/*ping_zhang:20090526 END*/
	...
	"</font></tr>\n", multilang("Select"), multilang("Device"), multilang(" Name"),
	multilang("Device Type"), multilang("Start Address"), multilang("End Address"), multilang("gateway address"),
	multilang("Reserved Option"));
	...
	for (i=0; i<entryNum; i++) {
		...
		{
		case CTC_Computer:
			devType = multilang("PC");
			break;
		case CTC_Camera:
			devType = multilang("Camera");
			break;
		case CTC_HGW:
			devType = multilang("HGW");
			break;
		case CTC_STB:
			devType = multilang("STB");
			break;
		case CTC_PHONE:
			devType = multilang("Phone");
			break;
		case CTC_UNKNOWN:
		default:
			devType = multilang("Unknown");
			break;
		}
