...
int ipPortFilterListV6(int eid, request * wp, int argc, char **argv)
{
	...
	multilang("Source"), multilang("IP Address"), 
#else
	multilang("Source"), multilang("Interface ID"), 
#endif
	multilang("Source Port"),
#ifdef CONFIG_IPV6_OLD_FILTER
	multilang("Destination"), multilang("IP Address"), 
#else
	multilang("Destination"), multilang("Interface ID"), 
#endif
	multilang("Destination Port"),
	...
	if (strAddIpPort[0] ) {		// IP/Port FILTER
		...
		if (strVal[0]) {
			...
			if (!inet_pton(PF_INET6, tmpBuf, &filterEntry.sIfId6Start)) {
				sprintf(tmpBuf, "Invalid sIfId6Start for ipport filter!");
				goto setErr_filter;
			}
		}
		...
		if (strVal[0]) {
			...
			if (!inet_pton(PF_INET6, tmpBuf, &filterEntry.sIfId6End)) {
				sprintf(tmpBuf,"Invalid sIfId6End for ipport filter!");
				goto setErr_filter;
			}
		}
		...
		if (strVal[0]) {
			...
			if (!inet_pton(PF_INET6, tmpBuf, &filterEntry.dIfId6Start)) {
				sprintf(tmpBuf,"Invalid dIfId6Start for ipport filter!");
				goto setErr_filter;
			}
		}
		...
		if (strVal[0]) {
			...
			if (!inet_pton(PF_INET6, tmpBuf, &filterEntry.dIfId6End)) {
				sprintf(tmpBuf,"Invalid dIfId6End for ipport filter!");
				goto setErr_filter;
			}
		}
#endif
