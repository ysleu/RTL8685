...
void formDhcpv6(request * wp, char *path, char *query)
{
	...
	if ( curDhcp == DHCP_LAN_SERVER ) {
		...
		if (str[0]) {
			...
			if (deleted <= 0) {
				strcpy(tmpBuf, "There is no item selected to delete(MIB_DHCPV6S_NAME_SERVER_TBL)!");
				goto setErr_dhcpv6;
			}
		...
		if (str[0]) {
			...
			if (intVal == 0) {
				...
				//return;
				strcpy(tmpBuf, "Error: Add Name Server chain record for DHCPv6.");
				goto setErr_dhcpv6;
			}
			else if (intVal == -1) {
		...
		if (str[0]) {
			...
			if (deleted <= 0) {
				strcpy(tmpBuf, "There is no item selected to delete(MIB_DHCPV6S_DOMAIN_SEARCH_TBL)!");
				goto setErr_dhcpv6;
			}
		...
		if (str[0]) {
			...
			if (intVal == 0) {
				//boaWrite(wp, "%s", "Error: Add Domain Blocking chain record.");
				//return;
				strcpy(tmpBuf, "Error: Add Domain chain record for DHCPv6.");
				goto setErr_dhcpv6;
			}
			else if (intVal == -1) {
				strcpy(tmpBuf, strTableFull);
		...
		if ( !mib_set(MIB_DHCPV6R_UPPER_IFINDEX, (void *)&upper_if)) {
			strcpy(tmpBuf, "Set MIB_DHCPV6R_UPPER_IFINDEX mib error!");
			goto setErr_dhcpv6;
		}
