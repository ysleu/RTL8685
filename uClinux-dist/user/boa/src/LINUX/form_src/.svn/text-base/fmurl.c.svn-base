	...
	if (str[0]) {
		...
		if (str[0]) {
			...
			if ( !mib_set(MIB_URL_CAPABILITY, (void *)&urlcap)) {
				strcpy(tmpBuf, "Set URL Capability error!");
				goto setErr_route;
			}
		}
		goto  setOk_route;
...
//		printf("str = %s\n", str);
		...
		if (intVal == 0) {
			...
			//return;
			strcpy(tmpBuf, "Error: Add URL chain record.");
			goto setErr_route;
		}
		else if (intVal == -1) {
	...
	if (str[0]) {
		...
		if (intVal == 0) {
			...
			//return;
			strcpy(tmpBuf, "Error: Add Keyword filtering chain record.");
			goto setErr_route;
		}
		else if (intVal == -1) {
			strcpy(tmpBuf, strTableFull);
...
int showURLTable(int eid, request * wp, int argc, char **argv)
{
	...
	multilang("Select"), multilang("FQDN"));
...
int showKeywdTable(int eid, request * wp, int argc, char **argv)
{
	...
	"<td align=center width=\"5%%\" bgcolor=\"#808080\">%s</td>\n"
	"<td align=center width=\"35%%\" bgcolor=\"#808080\">%s</td></font></tr>\n",
	multilang("Select"), multilang("Filtered Keyword"));
