...
// Magician: Commit immediately
	...
	if (strSet1p[0]){
		...
		{
			strcpy(tmpBuf, "Get 802.1p Mib error");
			goto setErr_qos;
		}
...
// Magician: Commit immediately
	...
	if (strSetPred[0]){
		...
		{
			strcpy(tmpBuf, "Get PRED Mib error");
			goto setErr_qos;
		}
...
#endif
#endif
	, multilang("Classification Rules")
#ifndef NEW_IP_QOS_SUPPORT
	, multilang("Classification Results")
#else
	, multilang("Mark")
#endif
	);
		...
		while ((token=strtok(arg0,","))!=NULL) {
			...
			for ( i; (i > old_itfid && i >= 0); i--) {
				...
				if (!mib_chain_get(MIB_IP_QOS_QUEUE_TBL, i, (void *)&entry)) {
					strcpy(tmpBuf, "Get chain record error 1!");
					goto setErr_queueadd;
				}
				entry.enable = 0;
			...
			if (!mib_chain_get(MIB_IP_QOS_QUEUE_TBL, itfid, (void *)&entry)){
				strcpy(tmpBuf, "Get chain record error 2!");
				goto setErr_queueadd;
			}
			entry.enable = 1;
...
int ipQosQueueList(int eid, request * wp, int argc, char **argv)
{
	...
	if ( type == IP_QOS_QUEUE_BUTTON ) {
		if ( entryNum > 0 ) {
			nBytesSent += boaWrite(wp,
				...
				multilang("RemoveQueue"), multilang("SaveAndReboot"));
		}
		return nBytesSent;
	}
	...
	if ( type == IP_QOS_QUEUE_LIST ) {
		nBytesSent += boaWrite(wp, "<tr><font size=2>"
      		...
      		multilang("Interface Name"), multilang("Description"),
		multilang("Precedence"), multilang("Queue Key"),
		multilang("Enable"), multilang("Remove"));
	}
