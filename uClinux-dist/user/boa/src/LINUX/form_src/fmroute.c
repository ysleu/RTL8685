		...
		for (i=0; i<totalEntry; i++) {
			...
			if (!gstrcmp(select, strBuf)) {
				if (mib_chain_get(MIB_IP_ROUTE_TBL, idx, (void *)&tmp)) {
					route_cfg_modify(&tmp, 1); // delete original route
				...
				route_cfg_modify(&entry, 0); // add new route
...
void formIPv6Route(request * wp, char *path, char *query)
{
	...
	if (!str && (entry.DstIfIndex == DUMMY_IFINDEX)) {
		...
		if (!inet_pton(AF_INET6, str, (struct in6_addr *)&tmp)) {
			snprintf(tmpBuf, 100, "Error: can't resolve next Hop %s", str);
			goto setErr_route;
		}
	}
	...
	if (str && str[0]) {
		...
		if (intVal == 0) {
			strcpy(tmpBuf, "This Route existed in MIB already!");
			goto setErr_route;
		}
...
void formRip(request * wp, char *path, char *query)
{
	...
	if (str[0]) {
		...
		if ( str[0]=='0' ) {
			...
			strcpy(tmpBuf, "Set RIP receive mode error!");
			goto setErr_rip;
		}
		...
		if ( str[0]=='0' ) {
			...
			strcpy(tmpBuf, "Set RIP send mode error!");
			goto setErr_rip;
		}
		...
		if (intVal == 0) {
			...
			//return;
			strcpy(tmpBuf, "Error: Add MIB_RIP_TBL chain record.");
			goto setErr_rip;
		}
		else if (intVal == -1) {
			strcpy(tmpBuf, strTableFull);
	...
	if (strVal[0] == '0') {//RIP
		...
		if (str[0]) {
			...
			if (intVal == 0) {
				...
				//return;
				strcpy(tmpBuf, "Error: Add MIB_OSPF_TBL chain record.");
				goto setErr_rip;
			}
			else if (intVal == -1) {
				strcpy(tmpBuf, strTableFull);
				...
				for (i=0; i<totalEntry; i++) {
					...
					if ( !gstrcmp(str, tmpBuf) ) {
						...
						if(mib_chain_delete(MIB_OSPF_TBL, idx) != 1) {
							strcpy(tmpBuf, "Delete MIB_OSPF_TBL chain record error!");
							goto setErr_rip;
						}
					}
				} // end of for
...
int showRipIf(int eid, request * wp, int argc, char **argv)
{
	...
	unsigned int entryNum, i;
	MIB_CE_RIP_T Entry;
	char ifname[IFNAMSIZ], *receive_mode, *send_mode;
	...
	multilang("Select"), multilang("Interface"),
	multilang("Receive Mode"), multilang("Send Mode"));
...
int showStaticRoute(int eid, request * wp, int argc, char **argv)
{
	...
	"<td align=center bgcolor=\"#808080\">%s</td>\n"
	"<td align=center bgcolor=\"#808080\">%s</td>\n"
	"</font></tr>\n", multilang("Select"), multilang("State"),
	multilang("Destination"), multilang("Subnet Mask"), multilang("Next Hop"),
	multilang("Metric"), multilang("Interface"));
...
int showIPv6StaticRoute(int eid, request * wp, int argc, char **argv)
{
	...
	"<td align=center bgcolor=\"#808080\">%s</td>\n"
	"</font></tr>\n", multilang("Select"), multilang("State"),
	multilang("Destination"), multilang("Next Hop"),
	multilang("Metric"), multilang("Interface"));
...
int routeIPv6List(int eid, request * wp, int argc, char **argv)
{
	...
	nBytesSent += boaWrite(wp, "<tr><font size=1>"
			...
			multilang("Destination"),  multilang("Next Hop"), multilang("Flags"),
			multilang("Metric"), multilang("Ref"), multilang("Use"), multilang("Interface"));
