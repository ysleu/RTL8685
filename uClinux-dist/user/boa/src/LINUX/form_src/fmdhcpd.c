...
//star: for dhcp change
	...
	if ( dhcp == DHCP_LAN_SERVER ) {
		...
		{
			strcpy(tmpBuf, "Invalid value of subnet mask!");
			goto setErr_dhcpd;
		}

		if((inPoolStart.s_addr & dhcpmask.s_addr) != (inPoolEnd.s_addr & dhcpmask.s_addr))
		{
			strcpy(tmpBuf, "DHCP IP range does not match under the  subnet mask!");
			goto setErr_dhcpd;
		}
		#endif
		...
		{
			strcpy(tmpBuf, "Set DHCP subnetmask failed!");
			goto setErr_dhcpd;
		}
		#endif
...
err:
	if (element == 0) {
		nBytesSent += boaWrite(wp,
			"<tr bgcolor=#b7b7b7><td><font size=2>%s</td><td><font size=2>----</td><td><font size=2>----</td></tr>", multilang("None"));
	}
	if (buf)
		free(buf);
