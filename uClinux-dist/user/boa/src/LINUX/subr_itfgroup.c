...
static void setup_wlan_group_member(int itf, int mbr)
{
	...
	{
		snprintf(s_ifname, 16, "%s-vap%d", getWlanIfName(), itf-WLAN_VAP_ITF_INDEX);  // ex: wlan0-vap0
...
// wlan1
	...
	for (i=1; i<IFGROUP_NUM; i++) {
		...
		if (mygroup == grpnum) {
			info[num].ifdomain = DOMAIN_WLAN;
			info[num].ifid=i;
			sprintf(info[num].name, "%s-vap%d", getWlanIfName() ,i-1);
			num++;
		}
	}
	...
	for (i=11; i<10+IFGROUP_NUM; i++)
	{
		...
		if (mygroup == grpnum) {
			info[num].ifdomain = DOMAIN_WLAN;
			info[num].ifid=i;
			sprintf(info[num].name, "%s-vap%d", getWlanIfName(), i-11);
			num++;
		}
	}
#endif
