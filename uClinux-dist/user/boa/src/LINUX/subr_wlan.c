...
#if (defined(CONFIG_RTL_8812_SUPPORT) && !defined(CONFIG_ENABLE_EFUSE)) || defined(WLAN_DUALBAND_CONCURRENT)
	...
	if(!(type & IWPRIV_HS)){
		if(type & IWPRIV_GETMIB){
			if(type & IWPRIV_INT) //int
				sprintf(parm, "%s=%u", s, value[0]);
			else //string
				sprintf(parm, "%s=%s", s, value);
		}
		else{
			if(type & IWPRIV_INT){ //int
				intVal = va_arg(ap, int);
				sprintf(parm, "%s=%u", s, intVal);
			}
			else{ //string
				s2 = va_arg(ap, char *);
				sprintf(parm, "%s=%s", s, s2);
			}
		}
	}
	else{
		...
		if(type & IWPRIV_HW2G){ //2G
			...
			if(type & IWPRIV_TXPOWER){
				...
				for(i=0; i<=177; i++) {
					sprintf(parm, "%s%02x", parm, pMib[i]);
				}
			}
...
void wl_updateSecurity(unsigned char band) {
	if (wl_isNband(band)) {
		...
		if (mib_get(MIB_WLAN_WPA_CIPHER_SUITE, &security)) {
			security &= ~(WPA_CIPHER_TKIP);
			mib_set(MIB_WLAN_WPA_CIPHER_SUITE, &security);
		}
		if (mib_get(MIB_WLAN_WPA2_CIPHER_SUITE, &security)) {
			security &= ~(WPA_CIPHER_TKIP);
			mib_set(MIB_WLAN_WPA2_CIPHER_SUITE, &security);
		}
	}
}
...
int setupWDS()
{
	...
	for(i=0;i<wds_num;i++){
		if (!mib_chain_get(MIB_WDS_TBL, i, (void *)&Entry))
			continue;
		sprintf(parm, "wds_add=%02x%02x%02x%02x%02x%02x,%u",
			Entry.macAddr[0], Entry.macAddr[1], Entry.macAddr[2],
	...
	if(wdsPrivacy == 1 || wdsPrivacy == 5){
		mib_get(MIB_WLAN_WDS_WEP_KEY, (void *)value);
		sprintf(parm, "wds_wepkey=%s", value);
		status|=va_cmd(IWPRIV, 3, 1, (char*)getWlanIfName(), "set_mib", parm);
		
	}
	else if(wdsPrivacy == 2|| wdsPrivacy == 4){
		mib_get(MIB_WLAN_WDS_PSK, (void *)value);
		sprintf(parm, "wds_passphrase=%s", value);
		status|=va_cmd(IWPRIV, 3, 1, (char*)getWlanIfName(), "set_mib", parm);
	}
	sprintf(parm, "wds_encrypt=%u", wdsPrivacy);
	status|=va_cmd(IWPRIV, 3, 1, (char*)getWlanIfName(), "set_mib", parm);
...
static int setupWLan_WPA(int vwlan_idx)
{
	...
	if (Entry.encrypt == WIFI_SEC_WEP)	// WEP mode
	{
		...
		if (Entry.wep == WEP64) {
			...
			// wepkey1
			// Mason Yu. 201009_new_security.
			sprintf(parm, "wepkey1=%02x%02x%02x%02x%02x", Entry.wep64Key1[0],
	...
	// ignoring it if using userland 'auth' security
		...
		//char wapiAuth;
		vInt = 0; // encmode = 0 for WAPI
		sprintf(parm, "wapiType=%d", Entry.wapiAuth);
		status|=va_cmd(IWPRIV, 3, 1, ifname, "set_mib", parm);
		...
		if (Entry.wapiAuth==2) { //PSK
			...
			if (Entry.wapiPskFormat) { // HEX
				...
				sprintf(parm, "wapiPsk=%s,%x", Entry.wapiPsk, Entry.wapiPskLen);
				status|=va_cmd(IWPRIV, 3, 1, ifname, "set_mib", parm);
			}
		} else { //AS
...
#ifdef WLAN_11W		
		if (Entry.encrypt == WIFI_SEC_WPA2){
			sprintf(parm, "dot11IEEE80211W=%d", Entry.dotIEEE80211W);
			status|=va_cmd(IWPRIV, 3, 1, ifname, "set_mib", parm);
			sprintf(parm, "enableSHA256=%d", Entry.sha256);
			status|=va_cmd(IWPRIV, 3, 1, ifname, "set_mib", parm);
		}
#endif
		if (!is8021xEnabled(vwlan_idx)) {
			// psk_enable: 1: WPA, 2: WPA2, 3: WPA+WPA2
			sprintf(parm, "psk_enable=%d", Entry.encrypt/2);
			status|=va_cmd(IWPRIV, 3, 1, ifname, "set_mib", parm);

			// passphrase
			sprintf(parm, "passphrase=%s", Entry.wpaPSK);
			status|=va_cmd(IWPRIV, 3, 1, ifname, "set_mib", parm);

			sprintf(parm, "gk_rekey=%d", Entry.wpaGroupRekeyTime);
			status|=va_cmd(IWPRIV, 3, 1, ifname, "set_mib", parm);
		}
		else {
			// psk_enable: 0
...
static int setupWLan_802_1x(int vwlan_idx)
{
	...
	{
		// 802_1x
		sprintf(parm, "802_1x=%u", Entry.enable1X);
		status|=va_cmd(IWPRIV, 3, 1, ifname, "set_mib", parm);
	}
#ifdef CONFIG_RTL_WAPI_SUPPORT
	else if (vInt == WIFI_SEC_WAPI) 
...
static void setupWLan_dot11_auth(struct wifi_mib* pmib, int vwlan_idx)
{
	...
	mib_get(MIB_WSC_DISABLE, (void *)&wsc_disabled);
...
#ifdef WLAN_MBSSID
		// VAP
		mib_get(MIB_WLAN_BLOCK_MBSSID, (void *)value);
		va_cmd(BRCTL, 3, 1, "mbssidblock", (char *)BRIF, value[0]==0?"0":"4");
#endif
...
#endif
	{
		...
		if(vwlan_idx==0){
			...
			if (value[0] == 0) {
				...
				mib_get(MIB_WLAN_FIX_RATE, (void *)&vInt);
			...
			pmib->dot11OperationEntry.block_relay = value[0];
			mib_get(MIB_WLAN_BLOCK_ETH2WIR, (void *)value);
#if defined(CONFIG_RTK_RG_INIT) && defined(WLAN_DUALBAND_CONCURRENT)
			rg_eth2wire_block(value[0]);
#else
			va_cmd("/bin/brctl", 3, 1, "wlanblock", BRIF, value[0]==0?"0":"2");
#endif
			...
			pmib->dot11QosEntry.dot11QosEnable = value[0];
#endif
		}
		else{
...
#if defined (CONFIG_RTL_8812_SUPPORT) || defined(WLAN_DUALBAND_CONCURRENT)
...
int setupWLan()
{
	...
	status|=iwpriv_cmd(IWPRIV_GETMIB, getWlanIfName(), "set_mib", "ssid", MIB_WLAN_SSID);
		...
		for (j=1; j<=4; j++) {
			if (!mib_chain_get(MIB_MBSSIB_TBL, j, (void *)&Entry)) {
 			...
 			//if ( Entry.wlanDisabled == 1 ) {
				sprintf(para2, "%s-vap%d", getWlanIfName(), j-1);
	...
	if (rpt_enabled) {
		...
		sprintf(para2, "%s-vxd", getWlanIfName());
		sprintf(parm, "ssid=%s", Entry.ssid);
		status|=va_cmd(IWPRIV, 3, 1, para2, "set_mib", parm);
		argv[1] = (char*)getWlanIfName();
	}
	...
	// autorate
	mib_get(MIB_WLAN_RATE_ADAPTIVE_ENABLED, (void *)value);
	autoRateRoot = (int)value[0];
	status|=iwpriv_cmd(IWPRIV_INT, getWlanIfName(), "set_mib", "autorate", autoRateRoot);
	...
	// hiddenAP
	status|=iwpriv_cmd(IWPRIV_GETMIB|IWPRIV_INT, getWlanIfName(), "set_mib", "hiddenAP", MIB_WLAN_HIDDEN_SSID);
	...
	if (autoRateRoot == 0)
	{
		...
		status|=iwpriv_cmd(IWPRIV_INT, getWlanIfName(), "set_mib", "fixrate", vInt);
	}
...
//12/23/04' hrchen, these MIBs are for CLIENT mode, disable them
	...
	*/
	...
	// block_relay
	status|=iwpriv_cmd(IWPRIV_GETMIB|IWPRIV_INT, getWlanIfName(), "set_mib", "block_relay", MIB_WLAN_BLOCK_RELAY);
...
#ifdef CONFIG_RTL_WAPI_SUPPORT
	mib_get(MIB_WLAN_WAPI_UCAST_REKETTYPE, (void *)&vChar);
	sprintf(parm, "wapiUCastKeyType=%d", vChar);
	status|=va_cmd(IWPRIV, 3, 1, (char *)getWlanIfName(), "set_mib", parm);
	if (vChar!=1) {
		mib_get(MIB_WLAN_WAPI_UCAST_TIME, (void *)&vInt);
		sprintf(parm, "wapiUCastKeyTimeout=%d", vInt);
		status|=va_cmd(IWPRIV, 3, 1, (char *)getWlanIfName(), "set_mib", parm);
		mib_get(MIB_WLAN_WAPI_UCAST_PACKETS, (void *)&vInt);
		sprintf(parm, "wapiUCastKeyPktNum=%d", vInt);
		status|=va_cmd(IWPRIV, 3, 1, (char *)getWlanIfName(), "set_mib", parm);
	}
	

	mib_get(MIB_WLAN_WAPI_MCAST_REKEYTYPE, (void *)&vChar);
	sprintf(parm, "wapiMCastKeyType=%d", vChar);
	status|=va_cmd(IWPRIV, 3, 1, (char *)getWlanIfName(), "set_mib", parm);
	if (vChar!=1) {
		mib_get(MIB_WLAN_WAPI_MCAST_TIME, (void *)&vInt);
		sprintf(parm, "wapiMCastKeyTimeout=%d", vInt);
		status|=va_cmd(IWPRIV, 3, 1, (char *)getWlanIfName(), "set_mib", parm);
		mib_get(MIB_WLAN_WAPI_MCAST_PACKETS, (void *)&vInt);
		sprintf(parm, "wapiMCastKeyPktNum=%d", vInt);
		status|=va_cmd(IWPRIV, 3, 1, (char *)getWlanIfName(), "set_mib", parm);
	}
#endif
	return status;
...
void enableFonSpot()
{
	...
	sprintf(str,"wlan0-vap%d", WLAN_MBSSID_NUM - 1);
	mib_get(MIB_ADSL_LAN_IP, (void *) buffer);
	sprintf(str2, "--dns1=%s --dns2=%s", inet_ntoa(*((struct in_addr *)buffer)), inet_ntoa(*((struct in_addr *)buffer)));
	mib_get(MIB_ELAN_MAC_ADDR, (void *)devAddr);
	sprintf(str3, "--radiusnasid=%02x-%02x-%02x-%02x-%02x-%02x",
...
//	if (both_band_ap == 1)
	...
	if (enableWscIf & 1) {
		retry = 0;
		sprintf(fifo_buf, WSCD_FIFO, WLANIF[0]);
	...
	if (enableWscIf & 2) {
		retry = 0;
		sprintf(fifo_buf, WSCD_FIFO, WLANIF[1]);
		while ((wscd_pid_fd = open((char *)fifo_buf, O_WRONLY|O_NONBLOCK)) == -1)
		{
			usleep(100000);
			retry ++;
...
static int wlanItfUpAndStartAuth(int vwlan_idx)
{
	...
	mib_get( MIB_WLAN_MODE, (void *)&wlanmode);
#ifdef WLAN_UNIVERSAL_REPEATER
	mib_get( MIB_REPEATER_ENABLED1, (void *)&rpt_enabled);
#endif
	...
	if(!(wan_mode & MODE_Wlan) || wisp_wan_id!=wlan_idx
		...
		if(wisp_wan_id == wlan_idx && (vwlan_idx == 0 || vwlan_idx == WLAN_REPEATER_ITF_INDEX))
...
#ifdef WLAN_1x
		sprintf(para_auth_conf, "/var/config/%s.conf", ifname);
		sprintf(para_auth_fifo, "/var/auth-%s.fifo", ifname);
...
int startWLan()
{
	...
	for(i = 0; i<NUM_WLAN_INTERFACE; i++){
		...
		if (wlan_mode == AP_MODE || wlan_mode == AP_WDS_MODE) {
			for (j=1; j<=WLAN_MBSSID_NUM; j++){
#ifdef WLAN_FAST_INIT
				sprintf(ifname, "%s-vap%d", getWlanIfName(), j-1);
				setupWLan(ifname, j);
#endif
		...
		if (wlan_mode != WDS_MODE) {
			...
			if (rpt_enabled){
#ifdef WLAN_FAST_INIT
				sprintf(ifname, "%s-vxd", getWlanIfName());
...
void getWispWanName(char *name)
{
	...
	if((wlan_mode == AP_MODE || wlan_mode == AP_WDS_MODE) && rptEnabled)
		sprintf(name, "wlan%d-vxd", wisp_wan_id);
	else if (wlan_mode == CLIENT_MODE)
		sprintf(name, "wlan%d", wisp_wan_id);
...
void setWlanDevFlag(char *ifname, int set_wan)
{
	...
	if(set_wan){
		sprintf(cmd_str, "echo \"%s write 0x2000\" > /proc/netdev_flag", ifname);
		system(cmd_str);
	}
	else{
		sprintf(cmd_str, "echo \"%s write 0x4000\" > /proc/netdev_flag", ifname);
		system(cmd_str);
		sprintf(cmd_str, "ifconfig %s 0.0.0.0", ifname);
		system(cmd_str);
	}
}
#endif
...
static int ping(char *ipaddr)
{
	char cmd[100];
	int status, ping_rtn;
	sprintf(cmd, "ping -w 2 %s > /dev/null", ipaddr);
