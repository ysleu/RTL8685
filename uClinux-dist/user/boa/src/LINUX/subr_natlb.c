...
int set_dhcp_NATLB_chain(int fh, char *nat_pre_name, char *nat_post_name)
{
	char buff[64];
...
int iptable_fw_natLB_dynamic( char *ifname, const char *proto, const char *extPort, const char *dstIP, const char *myip, char *nat_pre_name, char *nat_post_name, int fh)
{	
	unsigned int entryNum, i;
	MIB_CE_ATM_VC_T Entry;
...
void portfw_NATLB_dynamic( MIB_CE_PORT_FW_T *p, char *ifname, char *myip, char *nat_pre_name, char *nat_post_name, int fh)
{
	int hasExtPort=0;
	char * proto = 0;
	char intPort[32], extPort[32];
...
int iptable_dmz_natLB_dynamic(char *ifname, char *dstIP, char *myip, char *nat_pre_name, char *nat_post_name, int fh)
{	
	unsigned int entryNum, i;
	MIB_CE_ATM_VC_T Entry;
	unsigned char ipStr[20], subnetStr[20];
...
int set_dmz_natLB_dynamic(char *ifname, char *myip, char *nat_pre_name, char *nat_post_name, int fh)
{
	int vInt;
	unsigned char value[32];
	char ipaddr[32];	
...
int del_NATLB_dynamic(char *nat_pre_name, char *nat_post_name)
{
...
int set_ppp_NATLB_chain(char *nat_pre_name, char *nat_post_name)
{
	del_NATLB_dynamic(nat_pre_name, nat_post_name);
...
{	
	...
	for (i = 0; i < entryNum; i++) 
	{
		...
		if ( !Entry.enable || ((DHCP_T)Entry.ipDhcp != DHCP_DISABLED)
			|| ((REMOTE_ACCESS_T)Entry.cmode == ACC_BRIDGED)
			|| ((REMOTE_ACCESS_T)Entry.cmode == ACC_PPPOE)
