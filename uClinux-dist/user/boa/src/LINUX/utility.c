...
const char DNSV4RELAY_MARK_DHCP[] = "dnsv4_mangle_dhcp";
const char DNSV4RELAY_MARK_PPPOE[] = "dnsv4_mangle_pppoe";
const char DNSV4_SNAT[] = "dnsv4_snat";
#endif
...
#define MINIDLNAPID "/var/run/minidlna.pid"
...
*/
static int caculate_tblid_DNSRELAY(uint32_t ifid)
{
	int tbl_id;
	...
	if( MEDIA_INDEX(ifindex)==MEDIA_ETH )
	{
		if(PPP_INDEX(ifindex) == DUMMY_PPP_INDEX)
			tbl_id = ETH_INDEX(ifindex) + DNSRELAY_NAS_START;
		else
			tbl_id = PPP_INDEX(ifindex) + DNSRELAY_PPP_START;
	}
	else
#endif /*CONFIG_ETHWAN*/
#ifdef CONFIG_PTMWAN
	...
	{
		if(PPP_INDEX(ifindex) == DUMMY_PPP_INDEX)
			tbl_id = PTM_INDEX(ifindex) + DNSRELAY_PTM_START;
		else
			tbl_id = PPP_INDEX(ifindex) + DNSRELAY_PPP_START;
	}
	else
	...
	if (PPP_INDEX(ifindex) == DUMMY_PPP_INDEX)
		tbl_id = VC_INDEX(ifindex) + DNSRELAY_VC_START;
	else
		tbl_id = PPP_INDEX(ifindex) + DNSRELAY_PPP_START;
	}
...
#define DNSV4_DHCP_MARK_NUM	4
#define DNSV4_PPP_MARK_NUM		3
#endif
...
void __dev_setupIGMPSnoop(int flag)
{
	...
	if(flag){
#if !defined(CONFIG_LUNA)  //LUNA no more have /proc/br_igmpsnoop
...
#ifdef POLICY_ROUTING_DNSV4RELAY
	set_dhcp_dns_policy_route(fh, pEntry);
#endif
...
#ifdef POLICY_ROUTING_DNSV4RELAY
int set_dhcp_dns_policy_route(int fh, MIB_CE_ATM_VC_Tp pEntry)
{
	char buff[64];
	char buff2[128];
	int32_t tbl_id;
	char str_tblid[10]="";
	unsigned char Dnsmode, dhcp_num;
	char iproutecmd2[80];	// Mason Yu. for ACS and NTP
	char ifname[10];
	char mangle_name[30];

	ifGetName(pEntry->ifIndex, ifname, sizeof(ifname));
	snprintf(mangle_name, 30, "%s_%s", DNSV4RELAY_MARK_DHCP, ifname);

	mib_get(MIB_ADSL_WAN_DNS_MODE, (void *)&Dnsmode);
	// It is auto get DNSServer IP and this interface is not default route
	if ( Dnsmode == 0 && pEntry->dgw== 0)
	{
		tbl_id = caculate_tblid_DNSRELAY(pEntry->ifIndex);
		snprintf(str_tblid, sizeof(str_tblid), "%d", tbl_id);
		dhcp_num = VC_INDEX(pEntry->ifIndex)%DNSV4_DHCP_MARK_NUM;

		// (1) Flush mangle rule and delete IP rule and IP route
		//iptables -t mangle -F dnsv4_mangle_dhcp_vc0
		snprintf(buff, 64, "\tiptables -t mangle -F %s\n", mangle_name);
		WRITE_DHCPC_FILE(fh, buff);
		snprintf(buff2, 128, "\tiptables -t mangle -D OUTPUT -j %s\n", mangle_name);
		WRITE_DHCPC_FILE(fh, buff2);
		//iptables -t mangle -X dnsv4_mangle_dhcp_vc0
		snprintf(buff, 64, "\tiptables -t mangle -X %s\n", mangle_name);
		WRITE_DHCPC_FILE(fh, buff);
		//iptables -t mangle -N dnsv4_mangle_dhcp_vc0
		snprintf(buff, 64, "\tiptables -t mangle -N %s\n", mangle_name);
		WRITE_DHCPC_FILE(fh, buff);
		snprintf(buff2, 128, "\tiptables -t mangle -A OUTPUT -j %s\n", mangle_name);
		WRITE_DHCPC_FILE(fh, buff2);
		...
		snprintf(buff, 64, "\tip rule del fwmark %s table %s\n", rule_mark[dhcp_num], str_tblid);
		WRITE_DHCPC_FILE(fh, buff);

#if defined(CONFIG_USER_RTK_VOIP) && defined(CONFIG_USER_RTK_WAN_CTYPE)
		...
		// iptables -t nat -D dnsv4_snat -m mark --mark 0x20000/0xe0000 -o ptm0_1 -j MASQUERADE
		snprintf(buff2, 128, "\tiptables -t nat -D %s -m mark --mark %s -o %s -j MASQUERADE\n", DNSV4_SNAT, rule_mark[dhcp_num], ifname);
		WRITE_DHCPC_FILE(fh, buff2);

		// (2) Add default route for policy route table
		WRITE_DHCPC_FILE(fh, "if [ \"$router\" ]; then\n");
		//ip route add default dev vc0 via 192.168.1.254 table 64
		snprintf(iproutecmd2, 80, "\tip route add default dev $interface via $i table %s\n", str_tblid);
		WRITE_DHCPC_FILE(fh, "\tfor i in $router\n");
		WRITE_DHCPC_FILE(fh, "\tdo\n");
		WRITE_DHCPC_FILE(fh, iproutecmd2);
		WRITE_DHCPC_FILE(fh, "\tdone\n");
		WRITE_DHCPC_FILE(fh, "fi\n");
		...
		WRITE_DHCPC_FILE(fh, "\tdo\n");
		WRITE_DHCPC_FILE(fh, "\techo 'DNS=' $i\n");
		//iptables -t mangle -A dnsv4_mangle_dhcp_vc0 -p udp --dport 53 -d 192.168.88.20 -j MARK --set-mark 1
		snprintf(buff2, 128, "\tiptables -t mangle -A %s -p udp --dport 53 -d $i -j MARK --set-mark %s\n", mangle_name, rule_mark[dhcp_num]);
		WRITE_DHCPC_FILE(fh, buff2);
		WRITE_DHCPC_FILE(fh, "\tdone\n");
		WRITE_DHCPC_FILE(fh, "fi\n");

		// (4) Add IP rule and IP route and SNAT
		// ip rule add fwmark 1 table 200
		snprintf(buff, 64, "\tip rule add fwmark %s table %s\n", rule_mark[dhcp_num], str_tblid);
		WRITE_DHCPC_FILE(fh, buff);

		// Add net route for this policy route table
...
#if defined(CONFIG_USER_RTK_VOIP) && defined(CONFIG_USER_RTK_WAN_CTYPE)
		if(pEntry->applicationtype&X_CT_SRV_VOICE){
			snprintf(buff2, 128, "\tip rule add from $netip/$mask table %s\n", str_tblid);
			WRITE_DHCPC_FILE(fh, buff2);

		}
#endif

		// The local out will with error src IP(the interface of default gw) for policy route, so we should add SNAT rule.
		// iptables -t nat -D dnsv4_snat -m mark --mark 0x20000/0xe0000 -o ptm0_1 -j MASQUERADE
		snprintf(buff2, 128, "\tiptables -t nat -A %s -m mark --mark %s -o %s -j MASQUERADE\n", DNSV4_SNAT, rule_mark[dhcp_num], ifname);
		WRITE_DHCPC_FILE(fh, buff2);

		// (5) Add Lowest default route
		...
		// route del default dev ptm0_1 gw 192.168.8.2 metric 10000
		snprintf(buff2, 128, "\troute del default dev %s gw $router metric 10000\n", ifname);
		WRITE_DHCPC_FILE(fh, buff2);
		// route add default dev ptm0_1 gw 192.168.8.2 metric 10000
		snprintf(buff2, 128, "\troute add default dev %s gw $router metric 10000\n", ifname);
		WRITE_DHCPC_FILE(fh, buff2);
	}
	return 0;
}

int set_ppp_dns_policy_route(struct ppp_policy_route_info *ppp_info)
{
	int32_t tbl_id;
	unsigned int ifIndex;
	char str_tblid[10]="";
	char DnsIp[20]={0}, DnsIp2[20]={0},SourceRouteIp[20]={0};
	unsigned char Dnsmode, ppp_num;
	char mangle_name[30];
	char sourceroute_tmp[50];
	int applicationtype;

	mib_get(MIB_ADSL_WAN_DNS_MODE, (void *)&Dnsmode);
	// It is auto get DNSServer IP
	if ( Dnsmode == 0 ) {
		ifIndex = getIfIndexByName(ppp_info->if_name);
		applicationtype=getapplicationtypeByName(ppp_info->if_name);
		// Since we only have 3 mark value for PPP. See rule_mark_ppp[]
		ppp_num = PPP_INDEX(ifIndex)%DNSV4_PPP_MARK_NUM;
		tbl_id = caculate_tblid_DNSRELAY(ifIndex);
		snprintf(str_tblid, sizeof(str_tblid), "%d", tbl_id);
		snprintf(mangle_name, 30, "%s_%s", DNSV4RELAY_MARK_PPPOE, ppp_info->if_name);

		// (1) flush all policy route and rule for PPPoE
		//iptables -t mangle -F dnsv4_mangle_pppoe_ppp0
		va_cmd(IPTABLES, 4, 1, "-t", "mangle", "-F", mangle_name);
		va_cmd(IPTABLES, 6, 1, "-t", "mangle", "-D", "OUTPUT", "-j", mangle_name);
		va_cmd(IPTABLES, 4, 1, "-t", "mangle", "-X", mangle_name);
		va_cmd(IPTABLES, 4, 1, "-t", "mangle", "-N", mangle_name);
		va_cmd(IPTABLES, 6, 1, "-t", "mangle", "-A", "OUTPUT", "-j", mangle_name);
...
#if defined(CONFIG_USER_RTK_VOIP) && defined(CONFIG_USER_RTK_WAN_CTYPE)
		/* for voip interface, delete again */
		if(applicationtype&X_CT_SRV_VOICE){
			va_cmd("/bin/ip", 4, 1, "rule", "del", "table", str_tblid);
		}

#endif
		// iptables -t nat -D POSTROUTING -m mark --mark 0x20000/0xe0000 -o ptm0_1 -j MASQUERADE
		va_cmd("/bin/iptables", 12 , 1, "-t",  "nat", "-D", DNSV4_SNAT, "-m", "mark", "--mark", rule_mark_ppp[ppp_num], "-o", ppp_info->if_name, "-j", "MASQUERADE");

		// (2) set policy route and rule
		//strcpy(ip, inet_ntoa(*((struct in_addr *)&entry.yiaddr)));
		if (ppp_info->primary_dns) {
			strcpy(DnsIp,inet_ntoa( *((struct in_addr *)&ppp_info->primary_dns)) );
			// iptables -t mangle -A dnsv4_mangle_pppoe_ppp0 -p udp --dport 53 -d 192.168.88.20 -j MARK --set-mark 1
			va_cmd(IPTABLES, 14, 1, "-t", "mangle", "-A", mangle_name,
								"-p", "udp",
								"--dport", "53",
								"-d", DnsIp,
								"-j", "MARK",
								"--set-mark", rule_mark_ppp[ppp_num]);
		}

		if (ppp_info->second_dns) {
			strcpy(DnsIp2,inet_ntoa( *((struct in_addr *)&ppp_info->second_dns)) );
			// iptables -t mangle -A dnsv4_mangle_pppoe_ppp0 -p udp --dport 53 -d 192.168.88.20 -j MARK --set-mark 1
			va_cmd(IPTABLES, 14, 1, "-t", "mangle", "-A", mangle_name,
								"-p", "udp",
								"--dport", "53",
		...
		if (ppp_info->primary_dns || ppp_info->second_dns) {
			// iptables -t nat -A POSTROUTING -m mark --mark 0x20000/0xe0000 -o ptm0_1 -j MASQUERADE
			va_cmd("/bin/iptables", 12 , 1, "-t",  "nat", "-A", DNSV4_SNAT, "-m", "mark", "--mark", rule_mark_ppp[ppp_num], "-o", ppp_info->if_name, "-j", "MASQUERADE");
			// ip rule add fwmark 1 table 200
			va_cmd("/bin/ip", 6, 1, "rule", "add", "fwmark", rule_mark_ppp[ppp_num],
								"table", str_tblid);

#if defined(CONFIG_USER_RTK_VOIP) && defined(CONFIG_USER_RTK_WAN_CTYPE)
			if(applicationtype&X_CT_SRV_VOICE){
				strcpy(SourceRouteIp,inet_ntoa(*((struct in_addr *)&ppp_info->hisip)) );
				snprintf(sourceroute_tmp,50,"%s/%d",SourceRouteIp,24);
				va_cmd("/bin/ip", 6, 1, "rule", "add", "from", sourceroute_tmp,"table", str_tblid);
				// ip route add default dev ppp0 table 200
				va_cmd("/bin/ip", 7, 1, "route", "add", "default", "dev", ppp_info->if_name, "table", str_tblid);
			}
#endif
			// ip route add to 192.168.88.20 dev vc0 table 200
			va_cmd("/bin/ip", 8, 1, "route", "add", "to", DnsIp, "dev", ppp_info->if_name, "table", str_tblid);
			// ip route add to 192.168.88.21 dev vc0 table 200
			va_cmd("/bin/ip", 8, 1, "route", "add", "to", DnsIp2, "dev", ppp_info->if_name, "table", str_tblid);

			// Mason Yu.
			// Because policy route need default GW to take effect, so we set the default GW on main table but wiith lower priority.
			// It will not affect the real default GW.
			// route del default dev ptm0_1 gw 192.168.8.2 metric 10000
			va_cmd("/bin/route", 6, 1, "del", "default", "dev", ppp_info->if_name, "metric", "10000");
			// route add default dev ptm0_1 gw 192.168.8.2 metric 10000
			va_cmd("/bin/route", 6, 1, "add", "default", "dev", ppp_info->if_name, "metric", "10000");
		}
	}
	return 0;
}
...
#ifdef POLICY_ROUTING_DNSV4RELAY
// Added by Mason Yu
int setAll_Dnsv4_snat(int mode)
{
	unsigned int entryNum, i;
	MIB_CE_ATM_VC_T Entry;
	char ifname[IFNAMSIZ];
	unsigned char dhcp_num, ppp_num;
	...
	for (i = 0; i < entryNum; i++)
	{
		...
		if (mode == DELDNSROUTE)
		{
			if ((DHCP_T)Entry.ipDhcp == DHCP_CLIENT)
			{
				dhcp_num = VC_INDEX(Entry.ifIndex)%DNSV4_DHCP_MARK_NUM;
				// iptables -t nat -D POSTROUTING -m mark --mark 0x20000/0xe0000 -o ptm0_1 -j MASQUERADE
				va_cmd("/bin/iptables", 12 , 1, "-t",  "nat", "-D", DNSV4_SNAT, "-m", "mark", "--mark", rule_mark[dhcp_num], "-o", ifname, "-j", "MASQUERADE");
			}

			if (Entry.cmode == ACC_PPPOE || Entry.cmode == ACC_PPPOA)
			{
				ppp_num = PPP_INDEX(Entry.ifIndex)%DNSV4_PPP_MARK_NUM;
				// iptables -t nat -D POSTROUTING -m mark --mark 0x20000/0xe0000 -o ptm0_1 -j MASQUERADE
				va_cmd("/bin/iptables", 12 , 1, "-t",  "nat", "-D", DNSV4_SNAT, "-m", "mark", "--mark", rule_mark_ppp[ppp_num], "-o", ifname, "-j", "MASQUERADE");

			}
		}

		// (1) DNS is auto mode, (2) Not default GW (3) WAN channel is enable
		if (mode == ADDDNSROUTE && Dnsmode==0 && Entry.dgw== 0 && Entry.enable != 0)
		{
			if ((DHCP_T)Entry.ipDhcp == DHCP_CLIENT)
			{
				dhcp_num = VC_INDEX(Entry.ifIndex)%DNSV4_DHCP_MARK_NUM;
				// iptables -t nat -A POSTROUTING -m mark --mark 0x20000/0xe0000 -o ptm0_1 -j MASQUERADE
				va_cmd("/bin/iptables", 12 , 1, "-t",  "nat", "-A", DNSV4_SNAT, "-m", "mark", "--mark", rule_mark[dhcp_num], "-o", ifname, "-j", "MASQUERADE");
			}
			...
			if (Entry.cmode == ACC_PPPOE || Entry.cmode == ACC_PPPOA)
			{
				ppp_num = PPP_INDEX(Entry.ifIndex)%DNSV4_PPP_MARK_NUM;
				// iptables -t nat -A POSTROUTING -m mark --mark 0x20000/0xe0000 -o ptm0_1 -j MASQUERADE
				va_cmd("/bin/iptables", 12 , 1, "-t",  "nat", "-A", DNSV4_SNAT, "-m", "mark", "--mark", rule_mark_ppp[ppp_num], "-o", ifname, "-j", "MASQUERADE");
...
int clean_Dnsv4Relay_PolicyRoute(MIB_CE_ATM_VC_Tp pEntry)
{
	//char buff2[128];
	int32_t tbl_id;
	char str_tblid[10]="";
	...
	if ((DHCP_T)pEntry->ipDhcp == DHCP_CLIENT || pEntry->cmode == ACC_PPPOE || pEntry->cmode == ACC_PPPOA)
	{
		tbl_id = caculate_tblid_DNSRELAY(pEntry->ifIndex);
		snprintf(str_tblid, sizeof(str_tblid), "%d", tbl_id);
		...
		if (pEntry->cmode == ACC_PPPOE || pEntry->cmode == ACC_PPPOA) {
			snprintf(mangle_name, 30, "%s_%s", DNSV4RELAY_MARK_PPPOE, ifname);
		}

		//iptables -t mangle -F dnsv4_mangle_pppoe
		va_cmd(IPTABLES, 4, 1, "-t", "mangle", "-F", mangle_name);
		va_cmd(IPTABLES, 6, 1, "-t", "mangle", "-D", "OUTPUT", "-j", mangle_name);
		va_cmd(IPTABLES, 4, 1, "-t", "mangle", "-X", mangle_name);
		...
		va_cmd("/bin/ip", 4, 1, "rule", "del", "table", str_tblid);
#if defined(CONFIG_USER_RTK_VOIP) && defined(CONFIG_USER_RTK_WAN_CTYPE)
		// del VOIP source route
		va_cmd("/bin/ip", 4, 1, "rule", "del", "table", str_tblid);
#endif
		// route del default dev ptm0_1 metric 10000
		va_cmd("/bin/route", 6, 1, "del", "default", "dev", ifname, "metric", "10000");
	}
	return 0;
}

int cleanAll_Dnsv4Relay_PolicyRoute()
{
	unsigned int entryNum, i;
	MIB_CE_ATM_VC_T Entry;
	...
	for (i = 0; i < entryNum; i++) {
		/* Retrieve entry */
		if (!mib_chain_get(MIB_ATM_VC_TBL, i, (void *)&Entry)) {
			printf("cleanAll_Dnsv4Relay_PolicyRoute: cannot get ATM_VC_TBL(ch=%d) entry\n", i);
			return 0;
		}

		clean_Dnsv4Relay_PolicyRoute(&Entry);
	}
	setAll_Dnsv4_snat(DELDNSROUTE);
}
#endif
...
static int startIP_v4(char *inf, MIB_CE_ATM_VC_Tp pEntry, REMOTE_ACCESS_T ipEncap)
{
...
//			va_cmd(IFCONFIG, 4, 1, "nas0","up","promisc", myip);
			RG_add_ipoe(pEntry);
#endif
		...
		if (fp=fopen(ifup_path, "w+") ) {
...
#ifdef CONFIG_IPV6
			if(pEntry->IpProtocol == 0x1)
				fprintf(fp, "/bin/updateddctrl %s 1\n", wanif);
#else
			fprintf(fp, "/bin/updateddctrl %s 1\n", wanif);
#endif
#endif
			...
			if(ipv6Enable == 1)
			{
				...
				if (fp=fopen(ifup_path, "w+") ) {
					...
					fprintf(fp, "#!/bin/sh\n\n");
#ifdef CONFIG_USER_DDNS
					// Added by Mason Yu for Start updatedd.  Specify IP Address
					// only support IPv6 DDNS on dual stack.
					if(pEntry->IpProtocol == 0x3)
						fprintf(fp, "/bin/updateddctrl %s 3\n", wanif);
#endif
					...
					if ( pEntry->Ipv6Dhcp == 1) {
						...
						snprintf(value, 128, "/bin/dhclient -6 -sf /var/dhclient-script -lf %s -pf %s %s -d -q", leasefile, pidfile, wanif);
						// Request Address
...
static void addVCVirtualDev(MIB_CE_ATM_VC_Tp pEntry)
{
	...
	if (mType == MEDIA_ATM && (WAN_MODE & MODE_ATM)) {
		...
		if ((REMOTE_ACCESS_T)pEntry->cmode == ACC_BRIDGED)
...
static int setup_ATM_channel(MIB_CE_ATM_VC_Tp pEntry)
{
	...
	case ACC_PPPOA:
		// PPPoA
		printf("PPPoA\n");
		if (startPPP(0, pEntry, qosParms, ACC_PPPOA) == -1)
		{
...
// Mason Yu. enable_802_1p_090722
	...
	if (pEntry->cmode == ACC_BRIDGED || pEntry->cmode == ACC_MER ||
		pEntry->cmode == ACC_PPPOE || pEntry->cmode == ACC_6RD) {
		if (mType == MEDIA_ATM) {
			// mpoactl set vc0 vlan 1 pvid 1
			TRACE(STA_SCRIPT, "%s\n", msg.data);
	...
	else if ((REMOTE_ACCESS_T)pEntry->cmode == ACC_6RD)
	{
		// 6RD
		printf("6RD\n");
		if (startIP(wanif, pEntry, ACC_MER) == -1)
...
#ifdef CONFIG_RTK_RG_INIT
	mib_chain_update(MIB_ATM_VC_TBL, (void *)pEntry, mib_vc_idx);
#endif
...
static void addEthWANdev(MIB_CE_ATM_VC_Tp pEntry)
{
	...
	if( ((mType == MEDIA_ETH) && (WAN_MODE & MODE_Ethernet))
		...
		else if ((REMOTE_ACCESS_T)pEntry->cmode == ACC_MER || (REMOTE_ACCESS_T)pEntry->cmode == ACC_6RD)
...
#ifdef POLICY_ROUTING_DNSV4RELAY
	clean_Dnsv4Relay_PolicyRoute(pEntry);
#endif
...
#endif
#ifdef CONFIG_IPV6
			if (pEntry->IpProtocol & 0x1 && (DHCP_T)pEntry->ipDhcp != DHCP_DISABLED)
			{
				int dhcpc_pid;
				dhcpc_pid = read_pid((char*)DHCPC_PID);
				if (dhcpc_pid > 0)
...
#ifdef CONFIG_RTK_RG_INIT
	RG_WAN_Interface_Del(pEntry->ifIndex);
#endif
}
...
#ifdef POLICY_ROUTING_DNSV4RELAY
	//iptables -t mangle -F dnsv4_mangle_dhcp
	//iptables -t mangle -D OUTPUT -j dnsv4_mangle_dhcp
	//iptables -t mangle -X dnsv4_mangle_dhcp
	va_cmd(IPTABLES, 4, 1, "-t", "mangle", "-F", DNSV4RELAY_MARK_DHCP);
	va_cmd(IPTABLES, 6, 1, "-t", "mangle", "-D", "OUTPUT", "-j", DNSV4RELAY_MARK_DHCP);
	va_cmd(IPTABLES, 4, 1, "-t", "mangle", "-X", DNSV4RELAY_MARK_DHCP);
#endif
}
...
#ifdef POLICY_ROUTING_DNSV4RELAY
	// Mason Yu.
	// DNSV4RELAY_MARK_DHCP is for all local out packets. DNSV4RELAY_MARK_PPPOE is for special DNS request.
	//DNSV4RELAY_MARK_DHCP order must previous to DNSV4RELAY_MARK_PPPOE.
	mib_get(MIB_ADSL_WAN_DNS_MODE, (void *)&value);
	vInt = (int)(*(unsigned char *)value);
	// It is manual to  set DNSServer IP
	if ( vInt == 1 ) {
		va_cmd(IPTABLES, 4, 1, "-t", "mangle", "-N", DNSV4RELAY_MARK_DHCP);
		va_cmd(IPTABLES, 6, 1, "-t", "mangle", "-D", "OUTPUT", "-j", DNSV4RELAY_MARK_DHCP);
		va_cmd(IPTABLES, 6, 1, "-t", "mangle", "-A", "OUTPUT", "-j", DNSV4RELAY_MARK_DHCP);
	}

	//iptables -t nat -D POSTROUTING -j dnsv4_snat
	//iptables -t nat -A POSTROUTING -j dnsv4_snat
	va_cmd(IPTABLES, 6, 1, "-t", "nat", "-D", "POSTROUTING", "-j", DNSV4_SNAT);
	va_cmd(IPTABLES, 6, 1, "-t", "nat", "-A", "POSTROUTING", "-j", DNSV4_SNAT);
	setAll_Dnsv4_snat(DELDNSROUTE);
	// Sinces cleanAllFirewallRule() will clean all nat rule, so we should add dnsv4_snat rule again.
	setAll_Dnsv4_snat(ADDDNSROUTE);
#endif
...
static int startWanServiceDependency(int ipEnabled)
{
	...
	if(ipEnabled==0)
	{
		...
		if ( pid > 0)
		{
			printf("Bridge mode, kill the original radvd deamon.\n");
			kill(pid,SIGKILL);
	...
	if (ipEnabled)
	{
		...
		if (vChar != 0)
		{
			// setup radvd config and resync it if applicable
			setup_radvd_conf(1);
...
int startWan(int configAll, MIB_CE_ATM_VC_Tp pEntry)
{
	...
	char ifname[IFNAMSIZ];
	char vChar=-1;
#if defined(CONFIG_USER_DDNS) || defined(CONFIG_DEV_xDSL)
	Modem_LinkSpeed vLs;
	vLs.upstreamRate=0;
#endif
	ret = 1;
...
//#endif
	...
	// Mason Yu. Restart DDNSClient to update IP address to ddns server for static link.
#ifdef CONFIG_USER_DDNS
		#ifdef CONFIG_ETHWAN
...
/*ping_zhang:20090930 END*/
		...
		case MIB_WIFI_TEST:
		case MIB_WLAN_HIDDEN_SSID:
		case MIB_WLAN_WPA_CIPHER_SUITE:
		case MIB_WLAN_WPA2_CIPHER_SUITE:
...
#endif
		case MIB_WAN_MODE:
			buffer[0] = (unsigned char)WAN_MODE;
			sprintf(strbuf, "%u", buffer[0]);
			break;
		default:
			return -1;
	}
	...
	switch (id) {
		...
		case SYS_WLAN_ENCRYPT:
			if ( !mib_get( MIB_WLAN_ENCRYPT, (void *)buffer) )
				return -1;
			//strcpy(strbuf, wlan_encrypt[(WIFI_SECURITY_T)buffer[0]]);
			switch(buffer[0]) {
				case WIFI_SEC_NONE:
				case WIFI_SEC_WEP:
				case WIFI_SEC_WPA:
		...
		case SYS_WLAN_WPA_CIPHER:
			if ( !mib_get( MIB_WLAN_WPA_CIPHER_SUITE, (void *)buffer) )
				return -1;
			if ( buffer[0] == 0 )
				strcpy(strbuf, "");
			else
				strcpy(strbuf, wlan_Cipher[buffer[0]-1]);
			break;
		case SYS_WLAN_WPA2_CIPHER:
			if ( !mib_get( MIB_WLAN_WPA2_CIPHER_SUITE, (void *)buffer) )
				return -1;
			if ( buffer[0] == 0 )
				strcpy(strbuf, "");
			else
		...
		case SYS_WLAN_WPS_ENABLED:
			if (!mib_get(MIB_WSC_DISABLE, (void *)buffer))
				return -1;
...
int ifWanNum(const char *name)
{
	...
	MIB_CE_ATM_VC_T Entry;
	int type;
	unsigned char protocol = 0x3; // 1:IPv4, 2: IPv6, 3: Both. Mason Yu. MLD Proxy
...
// Mason Yu. MLD Proxy
#ifdef CONFIG_IPV6
		if ( protocol == 0x3 || Entry.IpProtocol == 0x3 || protocol == Entry.IpProtocol ) {
			if (type == 2) {
				if (Entry.cmode == ADSL_BR1483)
				{
					ifnum++;
...
#ifdef URL_BLOCKING_ON_BRIDGE_MODE
...
void itfcfg(char *if_name, int up_flag)
{
	...
	else if (strncmp(if_name, "ethphy", sizeof("ethphy"))==0) {  // nic IntPhy enable/disable
		fd = socket(AF_INET, SOCK_DGRAM, 0);
	  	if(fd< 0) {
	  		printf("Socket create failed !\n");
	  		return;
	  	}
		strncpy(ifr.ifr_name, ELANIF, IFNAMSIZ);
		ifr.ifr_data = (void *)&cmd;
		if (up_flag)
			cmd = 41; // enable
		else
			cmd = 42; // disable
		ioctl(fd, SIOCETHTEST, &ifr);
		close(fd);
	}
...
void applyPPtP(MIB_PPTP_T *pentry, int enable)
{
	struct data_to_pass_st msg;
	char index[5];
	char auth[20];
...
int applyL2TP(MIB_L2TP_T *pentry, int enable)
{
	struct data_to_pass_st msg;
	char index[5];
...
int getDisplayWanName(MIB_CE_ATM_VC_T *pEntry, char* name)
{
	...
	if (pEntry->cmode == ADSL_PPPoA) { // ppp0, ...
		...
		if (mType == MEDIA_ATM)
			...
			char wisp_wan[16];
			getWispWanName(wisp_wan);
			snprintf(name, 16, "ppp%u_%s", PPP_INDEX(pEntry->ifIndex), wisp_wan);
		}
#endif
		else
			return 0;
...
int getWanStatus(struct wstatus_info *sEntry, int max)
{
	...
	int linkState, dslState=0, ethState=0, wlanState=0;
	int i, ifindices[256], br_socket_fd;
	FILE *fp;
	...
	{
		entryNum = getWanEntrybyMedia(&tEntry, MEDIA_WLAN);
		if (entryNum>=0) {
			vcEntry[vccount].ifIndex = tEntry.ifIndex;
			ifGetName(tEntry.ifIndex, vcEntry[vccount].ifname, IFNAMSIZ);
			ifGetName(PHY_INTF(tEntry.ifIndex), vcEntry[vccount].devname, IFNAMSIZ);
	...
	char wisp_name[16];
	getWispWanName(wisp_name);
	wlanState = get_net_link_status(wisp_name);
#endif
	...
	for (i=0; i<ifcount; i++) {
		...
		if (sEntry[i].cmode == ACC_PPPOE || sEntry[i].cmode == ACC_PPPOA) {
...
void Internet_status(void)
{
	...
	strncpy(ifr1.ifr_name, ELANIF, IFNAMSIZ);
...
void DnsBindPvcRoute(int mode)
{
...
#ifdef POLICY_ROUTING_DNSV4RELAY
	char *operationMode2[2]={"-A","-D"};
#endif
	unsigned char DnsBindPvcEnable, Dnsmode;
	unsigned int DnsBindPvc1;
	...
	if (mode == DELDNSROUTE) {
		va_cmd(IPTABLES, 4, 1, "-t", "mangle", "-F", "dnsv4_mangle_dhcp");
		va_cmd(IPTABLES, 6, 1, "-t", "mangle", "-D", "OUTPUT", "-j", "dnsv4_mangle_dhcp");
		va_cmd(IPTABLES, 4, 1, "-t", "mangle", "-X", "dnsv4_mangle_dhcp");
	}

	// Mason Yu. only for manually setting
	if(DnsBindPvcEnable && Dnsmode)
	{
		if (mode == ADDDNSROUTE) {
			va_cmd(IPTABLES, 4, 1, "-t", "mangle", "-N", "dnsv4_mangle_dhcp");
			va_cmd(IPTABLES, 6, 1, "-t", "mangle", "-A", "OUTPUT", "-j", "dnsv4_mangle_dhcp");
		}
		...
		for(i=0;i<3;i++)
		{
			...
			if(dnsIP1.s_addr&&(DnsBindPvc1!=DUMMY_IFINDEX)){
				char interfaceName[IFNAMSIZ]={0};
				char DnsIp[20]={0};
#ifdef POLICY_ROUTING_DNSV4RELAY
				ifGetName(DnsBindPvc1, interfaceName, sizeof(interfaceName));
				//ifindex ==>  table index
				//if vc0's  ifindex is 0xff00 ==> table 0x21
				//if ppp0's ifindex is 0xff01 ==> table 0x31
				tbl_id = caculate_tblid_DNSRELAY(DnsBindPvc1);
				snprintf(str_tblid, sizeof(str_tblid), "%d", tbl_id);
				//aug_itoa(tbl_id, str_tblid, sizeof(str_tblid));
				strcpy(DnsIp,inet_ntoa(dnsIP1));

				// iptables -t mangle -A dnsv4_mangle_dhcp -p udp --dport 53 -d 192.168.88.20 -j MARK --set-mark 1
				va_cmd(IPTABLES, 14, 1, "-t", "mangle", operationMode2[mode], "dnsv4_mangle_dhcp",
									"-p", "udp",
									"--dport", "53",
									"-d", DnsIp,
									"-j", "MARK",
									"--set-mark", rule_mark[i]);

				// ip rule add fwmark 1 table 200
				va_cmd("/bin/ip", 6, 1, "rule", operationMode[mode], "fwmark", rule_mark[i],
									"table", str_tblid);

				// ip route add table 200 dev vc0
				// ip route add to 192.168.88.20 dev vc0 table 200
				va_cmd("/bin/ip", 8, 1, "route", operationMode[mode], "to", DnsIp, "dev", interfaceName, "table", str_tblid);
#else
...
#ifdef CONFIG_IPV6
void Dnsv6BindPvcRoute(int mode)
{
#ifdef DNSV6_BIND_PVC_SUPPORT
char *operationMode[2]={"add","del"};
unsigned char Dnsv6BindPvcEnable;
unsigned int Dnsv6BindPvc1;
struct in6_addr dnsv6IP1;
char* argv[8];
int i=0;
mib_get(MIB_DNSV6_BIND_PVC_ENABLE,(void *)&Dnsv6BindPvcEnable);
if(Dnsv6BindPvcEnable)
{
for(i=0;i<3;i++)
	{
		mib_get(MIB_ADSL_WAN_DNSV61+i, (void *)&dnsv6IP1);
		mib_get(MIB_DNSV6_BIND_PVC1+i,&Dnsv6BindPvc1);
		char interfaceName[IFNAMSIZ]={0};
		char strbuf[48]={0};

		if ( memcmp( dnsv6IP1.s6_addr, strbuf, 16) && (Dnsv6BindPvc1!=DUMMY_IFINDEX)) {
			//route -A inet6 add 2001::2/64 dev vc0
			argv[1]="-A";
			argv[2]="inet6";
			argv[3]=operationMode[mode];
			inet_ntop(PF_INET6, &dnsv6IP1, strbuf, 48);
			//printf("dnsv6ip=%s Dnsv6BindPvc1=%d\n",strbuf,Dnsv6BindPvc1);
			argv[4]=strbuf;
			argv[5]="dev";
			ifGetName(Dnsv6BindPvc1, interfaceName, sizeof(interfaceName));
			argv[6]=interfaceName;
			argv[7]=NULL;
			printf("/bin/route %s %s %s %s %s %s!\n",argv[1],argv[2],argv[3],argv[4],argv[5], argv[6]);
			do_cmd("/bin/route",argv,1);
		}
	}
}
...
void startSNAT(void)
{
	...
	for (i = 0; i < vcTotal; i++)
	{
		...
		if (((REMOTE_ACCESS_T)Entry.cmode == ACC_MER)  ||
		  ((REMOTE_ACCESS_T)Entry.cmode == ACC_ROUTED) ||
		  ((REMOTE_ACCESS_T)Entry.cmode == ACC_IPOA) )
		{
#ifdef CONFIG_IPV6
		...
		else if (((REMOTE_ACCESS_T)Entry.cmode == ACC_PPPOE) ||
		      ((REMOTE_ACCESS_T)Entry.cmode == ACC_PPPOA))
		{
#ifdef CONFIG_IPV6
			if (Entry.IpProtocol & 0x1) {
...
void startMiniDLNA(void)
{
	...
	createMiniDLNAconf("/var/minidlna.conf", "/mnt");
...
int reserve_critical_setting(void)
{
...
struct addrinfo *hostname_to_ip(char *hostname, IP_PROTOCOL IPVer)
{
	...
	hints.ai_family = IPVer == IPv4? AF_INET: IPVer == IPv6? AF_INET6: AF_UNSPEC;
...
int check_vlan_conflict(MIB_CE_ATM_VC_T *pEntry, int idx, char *err_msg)
{
	...
	for(i = 0 ; i < total ; i++)
	{
		...
		if((pEntry->cmode == ADSL_BR1483 && chk_entry.cmode == ADSL_BR1483)
			|| (pEntry->cmode != ADSL_BR1483 && chk_entry.cmode != ADSL_BR1483))
		{
