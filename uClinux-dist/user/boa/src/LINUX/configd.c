...
void *ddnsC(void *data)
{
	...
	while ((fp=fopen("/var/init_ok","r"))==NULL) {
		printf("ddnsC: System initiation do not finish.\n");
		sleep(3);
	}
	fclose(fp);
	...
	for (i=0; i<entryNum; i++) {
		...
		//printf("ddnsC: tinfo->ifname is %s. tinfo->ipversion is %d.\n", tinfo->ifname, tinfo->ipversion);
//		if ( (strcmp(tinfo->ifname, Entry.interface) == 0) || (strcmp(Entry.interface, "br0") == 0)
//		if ( (strcmp(Entry.interface, "all") == 0) || (strcmp(tinfo->ifname, Entry.interface) == 0) || (strcmp(Entry.interface, "br0") == 0)
//			|| (strcmp(tinfo->ifname, "all") == 0) ) {

		if ( (strcmp(tinfo->ifname, Entry.interface) != 0) && (strcmp(tinfo->ifname, "all") != 0) ) {
			printf("ddnsC: There is not for %s rule!!\n", tinfo->ifname);
			continue;
		}
		...
		// For IPv4
		if ( tinfo->ipversion == 0x1 || tinfo->ipversion == 0x3 ) {
			if (getInAddr(Entry.interface, IP_ADDR, (void *)&inAddr) == 1) {
				itfIP = inet_ntoa(inAddr);
				getV4IP = 1;
				printf("ddnsC: The %s IPv4 address is %s\n", Entry.interface, itfIP);
			}
			else {
		...
		//if ( tinfo->ipversion == 0x2 || tinfo->ipversion == 0x3 ) {
		if (tinfo->ipversion == 0x3 ) {
			// for IPv6 PPP dynamically
			if (strncmp(tinfo->ifname, "ppp", 3) == 0) {
				while ((getifip6(Entry.interface, IPV6_ADDR_UNICAST, ip6_addr, 6) ==0) && (num < 5))
				{
				...
				if ( num != 5) {
					...
					getV6IP = 1;
					printf("ddnsC: The %s IPv6 address is %s\n", Entry.interface, itfIPv6);
				}
			}
			// For static link or  all interface update
			else
			{
				count=getifip6(Entry.interface, IPV6_ADDR_UNICAST, ip6_addr, 6);
				if (count) {
						inet_ntop(PF_INET6, &ip6_addr[0].addr, itfIPv6, 256);
						getV6IP = 1;
						printf("ddnsC: The %s IPv6 address is %s\n", Entry.interface, itfIPv6);
		...
		if ( g_killprocess == KILL_PROCESS_OFF ) {
			...
			if (getV4IP && getV6IP)
				...
				va_cmd("/bin/updatedd", 8, 1, Entry.provider, "--", "-4", itfIP, "-6", itfIPv6, account, Entry.hostname);
			else if (getV4IP)
				// updatedd dyndns -- -4 61.11.22.33 yufc:663333 mason2.dyndns.info
				va_cmd("/bin/updatedd", 6, 1, Entry.provider, "--", "-4", itfIP, account, Entry.hostname);
			else if (getV6IP)
				// updatedd dyndns -- -6 2001:b010:7030:1a00:90:1a00:a2:8506 yufc:663333 mason2.dyndns.info
				va_cmd("/bin/updatedd", 6, 1, Entry.provider, "--", "-6", itfIPv6, account, Entry.hostname);
#else
			if (getV4IP)
				// updatedd dyndns -- -4 61.11.22.33 yufc:663333 mason2.dyndns.info
				va_cmd("/bin/updatedd", 6, 1, Entry.provider, "--", "-4", itfIP, account, Entry.hostname);
...
void startDdnsc(struct thread_info tinfo)
{
	pthread_t ptDdnscId;
	struct thread_info *data;

	//printf("startDdnsc: tinfo.ifname is %s. tinfo.ipversion is %d.\n", tinfo.ifname, tinfo.ipversion);
	data = calloc(1, sizeof(struct thread_info));
	data->ipversion = tinfo.ipversion;
	strcpy(data->ifname,tinfo.ifname);
	pthread_create(&ptDdnscId, NULL, ddnsC, data);

	pthread_detach(ptDdnscId);
}
...
inline int checkPidExist(int pid)
{
	int ret;
	ret = kill(pid,0);
...
void msg_handler() {
	...
	while (1) {
		...
		if (ret>0) {
			...
			msgProcess(&qbuf);
			isPidExisted = checkPidExist(qbuf.mtype);
			if (isPidExisted) { // Send the message only if the receiving process alive.
#ifdef _SET_MSGQ_SIZE_AND_NONBLOCK_
				send_message_nonblock(msgqueue_id, qbuf.mtype, qbuf.request, &qbuf.msg);
#else
