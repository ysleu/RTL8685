...
int xml_mib_load(CONFIG_DATA_T type, CONFIG_MIB_T flag)
{
	...
	printf("%s():...\n", __FUNCTION__);
	if (va_cmd("/bin/sh",3,1, "/etc/scripts/config_xmlconfig.sh", "-l", current_file) != MSG_SUCC) {
...
int xml_mib_backup(CONFIG_MIB_T type)
{
	int ret=1;

	printf("%s():...\n", __FUNCTION__);
	if (va_cmd("/bin/sh",3,1, "/etc/scripts/config_xmlconfig.sh", "-s", backup_file) != MSG_SUCC) {
		printf("[xmlconfig] mib backup table failed\n");
		ret = 0;
	}else 
		printf("[xmlconfig] mib backup table success\n");
	
	return ret;
}

int xml_mib_restore(CONFIG_MIB_T type)
{
	int ret=1;

	printf("%s():...\n", __FUNCTION__);
	if (va_cmd("/bin/sh",3,1, "/etc/scripts/config_xmlconfig.sh", "-l", backup_file) != MSG_SUCC) {
		printf("[xmlconfig] mib restore table failed\n");
		ret = 0;
	}else 
		printf("[xmlconfig] mib restore table success\n");
...
int xml_mib_flash_to_default(CONFIG_DATA_T type)
{
	...
	printf("%s():...\n", __FUNCTION__);
	if (va_cmd("/bin/sh",2,1, "/etc/scripts/config_xmlconfig.sh", "-d") != MSG_SUCC) {
...
/**
 *Call if PPP & DHCP gateway changed
 */
int cmd_ip_config_changed()
{
	struct mymsgbuf qbuf;
	MSG_T *mymsg;
	int ret = 1;
	...
	mymsg = &qbuf.msg;
	mymsg->cmd = CMD_IP_CONFIG_CHANGED;
	sendcmd(&qbuf);
	if (qbuf.request != MSG_SUCC) {
		printf("%s failed\n", __FUNCTION__);
		ret = 0;
