...
void ShowIPv6()
{
	...
	for( i = 0; i < ifcount; i++ )
	{
		if (sEntry[i].cmode == ACC_BRIDGED || (sEntry[i].ipver != 0 && sEntry[i].ipver != 2))
...
void setWlanAdv()
{
	...
	while (1)
	{
		...
		case 1://(1) Set
			...
			mib_set(MIB_WLAN_FIX_RATE, (void *)&usInt);
			...
			if (hiddenSSID && wscUseVersion != 0) // force to disable WPS
				mib_set(MIB_WSC_DISABLE, (void *)&hiddenSSID);
			#endif
...
void setWlanSecurity()
{
	...
	while (1)
	{
		...
		mib_get( MIB_WLAN_ENCRYPT,  (void *)&enct);
		...
		case 1://(1) Set
			...
			if (disableWps && wpsUseVersion != 0) // force to disabled
				mib_set(MIB_WSC_DISABLE, (void *)&disableWps);
			#endif //WPS20
			...
			if (enct == WIFI_SEC_WEP) {
				...
				if(enableRS == 0 )
				{
					...
					mib_set( MIB_WLAN_AUTH_TYPE, (void *)&auth); // Mason Yu. 201009_new_security
...
#ifndef NEW_WIFI_SEC
					case CLI_ENC_WPA:
						mib_set(MIB_WLAN_WPA_CIPHER_SUITE, &unicastCipher);
						break;
#endif
					case CLI_ENC_WPA2:
						mib_set(MIB_WLAN_WPA2_CIPHER_SUITE, &wpa2UnicastCipher);
						break;
					case CLI_ENC_WPA2_MIXED:
						mib_set(MIB_WLAN_WPA_CIPHER_SUITE, &unicastCipher);
						mib_set(MIB_WLAN_WPA2_CIPHER_SUITE, &wpa2UnicastCipher);
...
#endif
			mib_set(MIB_WLAN_ENCRYPT, (void *)&enct2save);
#ifdef CONFIG_WIFI_SIMPLE_CONFIG//WPS def WIFI_SIMPLE_CONFIG
			update_wps_configured(0);
#endif
			config_WLAN(ACT_RESTART);
...
void ShowKeyInfo()
{
	...
	else
	{
		if (vChar ==0 || vChar == WSC_ENCRYPT_NONE)
			...
			mib_get(MIB_WSC_PSK, (void *)keystr);
	}
	...
	while (1)
	{
		...
		switch(snum)
		{
			...
			case 2:  // (2) Regenerate PIN
				mib_get(MIB_WSC_DISABLE, (void *)&vChar);
				if(vChar == 0)	 // WPS enabled
				{
					do
					{
...
void setWANMode()
{
	...
	int num, min, max;
	unsigned char cur_wan_mode = (unsigned char)WAN_MODE;
...
int addEthWanConfiguration(MEDIA_TYPE_T m_type)
{
	...
	entry.ifIndex = TO_IFINDEX(m_type, (entry.cmode==ACC_PPPOE?ppp_index:DUMMY_PPP_INDEX), vc_index);	
...
int delEthWanConfiguration(MEDIA_TYPE_T m_type)
{
	...
	if(mib_chain_get(MIB_ATM_VC_TBL,idxArray[index],&PvcEntry)&&(PvcEntry.cmode!=ADSL_BR1483))
	{
...
#ifdef _PRMT_X_TELEFONICA_ES_DHCPOPTION_
	if ((tmpEntry.cmode == ADSL_MER1483) && (tmpEntry.ipDhcp == DHCP_CLIENT))
...
retry:
	...
	if (entry->cmode != ADSL_BR1483) {
		...
		if(entry->cmode == ADSL_DSLite)
		{
			entry->AddrMode |= 4;
#ifdef CONFIG_IPV6
			entry->IpProtocol = 0x2; 	// IPv6 only
#endif
		}
		else
		{
	...
	//		if (entry->pppCtype != CONTINUOUS) {
		...
		if(entry->IpProtocol & 0x2)
		{
			...
			if(entry->cmode == ADSL_DSLite)
			{
				...
				entry->cmode = ACC_MER; // DS-Lite only for MER
			}
			else
#endif
			{
...
int setNewPortMapping(int mType)
{
	...
	for( i = 0; i < total_entries; i++ )
	{
		...
		if (vc_entry.cmode == ADSL_PPPoE)
			mode = "PPPoE";
		else if (vc_entry.cmode == ADSL_PPPoA)
			mode = "PPPoA";
		else if (vc_entry.cmode == ADSL_BR1483)
			mode = "br1483";
		else if (vc_entry.cmode == ADSL_MER1483)
			mode = "mer1483";
		else if (vc_entry.cmode == ADSL_RT1483)
			mode = "rt1483";
#ifdef CONFIG_ATM_CLIP
		else if (vc_entry.cmode == ADSL_RT1577)
			mode = "rt1577";
#endif
#ifdef DUAL_STACK_LITE
...
int modifyATMSettings()
{
	...
	CLEAR;
	max = cli_atmVcList(idx2entry); //_wanConfList(1);
	printf("\n\n");
...
int setSNTPServer(){
	...
	if ( sel == 1 ) {
		...
		if ( mib_set(MIB_NTP_SERVER_HOST1, NTPServer[sel_list - 1]) == 0) {
