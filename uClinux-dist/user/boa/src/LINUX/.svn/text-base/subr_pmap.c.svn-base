	...
	for(j = PMAP_ETH0_SW0; j <= PMAP_ETH0_SW3 && j < SW_LAN_PORT_NUM; ++j)
	{
		if(BIT_IS_SET(fgroup, j))
		{
			...
			if(BIT_IS_SET(itfGroup, j)) {
				...
				if ((pEntry->cmode == ADSL_BR1483) && (pEntry->applicationtype & X_CT_SRV_OTHER)) {
	...
	for(j = PMAP_WLAN0; j <= PMAP_WLAN1_VAP3; ++j)
	{
	    if(BIT_IS_SET(fgroup, j))
	    {
			...
			if(BIT_IS_SET(itfGroup, j)) {
				...
				// don't touch my dhcp server if mapping to IPTV type
				// iptables -A inacc -i $WLAN_IF -p UDP --dport 67 -j DROP
				if ((pEntry->cmode == ADSL_BR1483) && (pEntry->applicationtype & X_CT_SRV_OTHER)) {
					//printf("***** Set drop dhcp rule for %s\n", wlan[j - PMAP_WLAN0]);
					va_cmd(IPTABLES, 10, 1, (char *)FW_ADD, (char *)FW_INACC,
	...
	for(i = 0; i < totalEntry; ++i)
	{
		if(mib_chain_get(MIB_ATM_VC_TBL, i, (void*)&temp_Entry))
		{
			if (!temp_Entry.enable)
				continue;
			if((temp_Entry.cmode == ADSL_BR1483 ||
				temp_Entry.cmode == ADSL_MER1483 ||
				temp_Entry.cmode == ADSL_PPPoE) &&
			...
			if(temp_Entry.cmode == ADSL_MER1483 || temp_Entry.cmode == ADSL_PPPoE
					|| temp_Entry.cmode == ADSL_RT1483)
			{
				if(!iptbl_done_once)
				{
