...
//   fprintf (Ap, "Month run            %d/%d\n", date.month, date.year);
   ...
   fprintf(Ap,"%s\n",Reg_Define);
   fprintf (Ap, "\n");
   fprintf(Ap,"Microseconds 1 loop:  %12.2ld\n",Microseconds);
   fprintf(Ap,"Dhrystones / second:  %10.0ld\n",Dhrystones_Per_Second);
   fprintf(Ap,"VAX MIPS rating:      %12.2ld\n\n",Vax_Mips);
//   fclose(Ap);
   }
...
extern void save_counter(unsigned int* cnt0, unsigned int* cnt1, unsigned int* cnt2, unsigned int* cnt3);

//double my_clock(void)
unsigned int my_clock(void)
{
	unsigned int cnt0[2];	
	unsigned int cnt1[2];	
	unsigned int cnt2[2];	
	...
	save_counter( cnt0, cnt1, cnt2, cnt3);

	//return (((double)cnt0[0])*(double)(0x100000000)+((double)cnt0[1]));
	return (cnt0[1]);
}
