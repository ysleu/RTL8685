...
int do_mgr_VOIP_MGR_SET_SESSION( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	int ret = 0;
	extern unsigned int CurrentRfc2833Mode[];
	...
	if (rfc2833_payload_type_local[s_id]!=0 && rfc2833_payload_type_remote[s_id]!=0)
		CurrentRfc2833Mode[stVoipMgrSession.ch_id] = 1;
	else
		CurrentRfc2833Mode[stVoipMgrSession.ch_id] = 0;
#endif
