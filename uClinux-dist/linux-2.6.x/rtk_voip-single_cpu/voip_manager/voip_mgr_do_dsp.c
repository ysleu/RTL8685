...
int do_mgr_VOIP_MGR_SET_VAD_CNG_THRESHOLD( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	extern int G711_SetThresholdVADCNG( int32 sid, int VADthres, int CNGthres, int32 CNGmode, int32 CNGlevel, int32 CNGgain );
	uint32 s_id;
	...
	G711_SetThresholdVADCNG( s_id, stVoipThresVadCngConfig.nThresVAD, stVoipThresVadCngConfig.nThresCNG,
				  stVoipThresVadCngConfig.nCNGMode, stVoipThresVadCngConfig.nCNGLevel, stVoipThresVadCngConfig.nCNGGain );
...
int do_mgr_VOIP_MGR_SET_ECHO_TAIL_LENGTH( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	// Host auto forward 
#else
	if (stVoipValue.value1 == 1) //NLP on
		nlp = LEC_NLP;
	else
		nlp = 0;
...
//#endif
	...
	LEC_g168_init(3, LEC|nlp);// for handset
		#endif
	#else
	...
	#ifdef CONFIG_RTK_VOIP_DRIVERS_IP_PHONE
		#ifdef T_TYPE_ECHO_CAN
	LEC_g168_init(stVoipValue.ch_id, LEC|nlp);
		#else
...
#if ! defined (AUDIOCODES_VOIP)
void AEC_g168(unsigned int chid, int16_t *pRin, int16_t *pSin, int16_t *pEx);
...
int do_mgr_VOIP_MGR_DTMF_DET_THRESHOLD( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	// Host auto forward 
#else
	dtmf_det_threshold_set(stVoipCfg.ch_id, stVoipCfg.cfg, stVoipCfg.cfg2);
#endif
	return 0;
}
#else
...
#endif
	if(stVoipCfg.enable == 1 || stVoipCfg.enable == 2)
		dtmf_start((char)stVoipCfg.ch_id, stVoipCfg.cfg);
	else if ( stVoipCfg.enable == 0)
		dtmf_stop((char) stVoipCfg.ch_id, stVoipCfg.cfg);
#endif
	return 0;
}
#else
...
int do_mgr_VOIP_MGR_DTMF_GET( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	COPY_FROM_USER(&stVoipValue, (TstVoipValue *)user, sizeof(TstVoipValue));
	save_flags(flags); cli();
	dtmf_out(stVoipValue.ch_id, &stVoipValue.value, stVoipValue.value1);
	restore_flags(flags);
...
/**
 * @ingroup VOIP_DSP_DTMF
 * @brief Retrieve Received RFC2833 event
 * @param TstVoipValue.ch_id Channel ID
 * @param TstVoipValue.m_id Media ID 
 * @param [out] TstVoipValue.value A Received RFC2833 Event. If output is (-1), it means no event in FIFO.
 * @see VOIP_MGR_RFC2833_RX_GET TstVoipValue 
 */
#if ! defined (AUDIOCODES_VOIP)
int do_mgr_VOIP_MGR_RFC2833_RX_GET( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	uint32 s_id;
	extern RFC2833_EVENT rfc2833_rx_out(uint32 s_id);
	...
	s_id = API_GetSid(stVoipValue.ch_id, stVoipValue.m_id);
	save_flags(flags); cli();
	stVoipValue.value = rfc2833_rx_out(s_id);
	restore_flags(flags);
	//if (stVoipValue.value != (-1))
		//PRINT_MSG("VOIP_MGR_RFC2833_RX_GET: %d, sid%d\n", stVoipValue.value, s_id);
	return COPY_TO_USER(user, &stVoipValue, sizeof(TstVoipValue), cmd, seq_no);
}
#else
int do_mgr_VOIP_MGR_RFC2833_RX_GET( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	save_flags(flags); cli();
	//stVoipValue.value = rfc2833_rx_out(s_id);
	// Not support for ACMW
	stVoipValue.value = -1; // no event
	restore_flags(flags);
	return COPY_TO_USER(user, &stVoipValue, sizeof(TstVoipValue), cmd, seq_no);
}
#endif
...
int do_mgr_VOIP_MGR_SEND_RFC2833_PKT_CFG( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	{
		extern unsigned char send_2833_count_down[MAX_SESS_NUM];
		extern int send_dtmf_flag[];
		extern int g_digit[MAX_VOIP_CH_NUM];
		
		send_dtmf_flag[ stRFC2833.sid ] = 1;
		g_digit[ stRFC2833.ch_id ] = stRFC2833.digit;
		send_2833_count_down[ stRFC2833.sid ] = stRFC2833.duration / ( PCM_PERIOD * 10 );
		PRINT_R("set send_2833_count_down[%d] to %d\n", stRFC2833.sid, send_2833_count_down[ stRFC2833.sid ]);
