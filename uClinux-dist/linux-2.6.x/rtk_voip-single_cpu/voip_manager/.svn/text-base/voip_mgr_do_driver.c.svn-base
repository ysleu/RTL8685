...
int do_mgr_VOIP_MGR_FLUSH_FIFO( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	extern void flush_dtmf_fifo(uint32 ch_id, unsigned int dir);
	...
	flush_dtmf_fifo(stVoipValue.ch_id, 0);
	flush_dtmf_fifo(stVoipValue.ch_id, 1);
	restore_flags(flags);
#endif
	return 0;
}
...
int do_mgr_VOIP_MGR_FXO_OFF_HOOK( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	// Host auto forward 
#else
	flush_dtmf_fifo(stVoipCfg.ch_id, 0);//To avoid error events from other phone of the same PSTN line
	flush_dtmf_fifo(stVoipCfg.ch_id, 1);
...
#ifdef CONFIG_RTK_VOIP_ETHERNET_DSP_IS_HOST
	return -ENXIO;;		// pkshih: This is due to old architecture 
#else	
