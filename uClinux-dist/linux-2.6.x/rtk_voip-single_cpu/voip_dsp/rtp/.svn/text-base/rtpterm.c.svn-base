...
static uint32 Support_FaxModem_RFC2833RxPlay[VOIP_CH_NUM] = {0};
		...
		if(!eventPayload->edge)	/* not edge */
		{
			...
			{						
...
#endif
				{
					...
					extern int rfc2833_rx_in(uint32 s_id, RFC2833_EVENT input);
					rfc2833_rx_in(sid, eventPayload->event);
				}
			...
			RtpSeqNumber seqNo, RtpTime timestamp )
{
	...
	if( ( ( RtpRedPrimaryHeader * )pSrcHeader ) ->blockPT 
...
int32 RtpTerminal_Read(uint32* pchid, uint32* psid, uint8 *pBuf, int32 nSize)
{
	...
	if(p)
	{
		...
		*psid = p->sid;
		...
		if( rfc2833_payload_type_local[sid] )
		{
...
int32 RtpTerminal_Write(uint32 chid, uint32 sid, uint8 *pBuf, int32 nSize)
{
	...
	if (rfc2833_payload_type_local[sid]!=0 && rfc2833_payload_type_remote[sid]!=0)
	{
