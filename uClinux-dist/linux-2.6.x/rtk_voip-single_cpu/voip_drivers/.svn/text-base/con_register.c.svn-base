...
#if defined(CONFIG_RTK_VOIP_PLATFORM_8686) || defined(CONFIG_RTK_VOIP_PACKAGE_867X)
	while ( (TS == 0) || ( TS == 6) ){
		TS += 2;
	}
//	if ( (TS == 0) || (TS == 6) )
	...
	case REG_PROC_COOKIE_SND:
		{
			...
			n += sprintf( buf + n, "sch name    TS1/2 T flgs \n--- ------- ----- - ----\n" );
			
			p_snd = ( const voip_snd_t * )voip_snd_link.head;
			
			while( p_snd ) {
				n += sprintf( buf + n, "%3d %-7s %2d/%2d %d %04X\n", 
					p_snd ->sch,
...
{
	remove_proc_entry( PROC_VOIP_DIR "/" PROC_VOIP_CH_MAPS_DIR "/ioc", NULL );
