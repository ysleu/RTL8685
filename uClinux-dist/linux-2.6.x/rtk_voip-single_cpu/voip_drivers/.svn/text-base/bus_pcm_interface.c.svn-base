...
extern unsigned long sound_c1_8k_rin5, sound_c1_8k_rin10, sound_c1_8k_rin15, sound_c1_8k_rin20, sound_c1_8k_rin30;
short* ec128_pattern[] = {&sound_c1_8k_rin5, &sound_c1_8k_rin10, &sound_c1_8k_rin15, &sound_c1_8k_rin20, &sound_c1_8k_rin30};
...
#if !defined(CONFIG_RTK_VOIP_DRIVERS_PCM865xC) && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM8672) && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM8972B_FAMILY) && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM89xxC) && !defined(CONFIG_RTK_VOIP_DRIVERS_PCM8676) 
	switch(type)
	{
		case P0OK:
...
#endif
			{
				...
				if (chanEnabled[chid] == TRUE)	{
								...
								p_bus, ( uint16 * )txbuf ) == NULL )
					{
						...
						if (stVoipdataget[bch].write_enable&32)
						{
							ec128_start_count++;
							if (ec128_start_count>200) {
								...
								for (ii=0; ii < PCM_PERIOD; ii++)
								{
									for (k=0; k<80; k++)
									{
										*((short*)txbuf+i*(PCM_PERIOD_10MS_SIZE/2)+k) = ec128_pattern[4][ec128_pattern_index++];
								...
								for (ii=0; ii < PCM_PERIOD; ii++)
								{
									for (k=0; k<80; k++)
									{
										*((short*)txbuf+i*(PCM_PERIOD_10MS_SIZE/2)+k) = 0;
									}
									ec128_pattern_index=0;
								}
	...
	if(pcmctrl_devices)
	{
		...
		for (ch = 0; ch <BUS_PCM_CH_NUM; ch++)
		{
			...
			switch(law)
			{
				...
				#if defined(CONFIG_RTK_VOIP_DRIVERS_PCM8672) || defined(CONFIG_RTK_VOIP_DRIVERS_PCM8972B_FAMILY) || defined(CONFIG_RTK_VOIP_DRIVERS_PCM89xxC) || defined(CONFIG_RTK_VOIP_DRIVERS_PCM8676)
