...
unsigned char dtmf_removal[MAX_VOIP_CH_NUM][2/*dir*/] = {[0 ... MAX_VOIP_CH_NUM-1] = {0, 0}};// declare for dtmf removal, 0: disable  1: enable (value is assigned to 1 when dtmf tone is detected, otherwise, 0!)
...
int send_dtmf_flag[MAX_SESS_NUM] = {0};
...
Word16 delayedecho_buf[MAX_VOIP_CH_NUM][1024+80];
#endif
...
static int32 PCM_RX(uint32 chid, uint32 *rxBuf, const uint16 *lec_ref)
{
	...
	extern int dtmf_in(uint32 ch_id, char input, unsigned int dir);
	...
	/**********************************************
		...
		if (dtmf_chid[chid][0] == 1)
		{
			...
			extern long dtmf_det_threshold[MAX_VOIP_CH_NUM][2];
		...
		if ( (fsk_alert_done[chid] == 1) && (dtmf_digit.digit == gFskType2AckTone[chid]))
		{
			...
			dtmf_in(chid, dtmf_digit.digit, 0);
	...
	/**********************************************                 
		...
		extern uint32* chanInfo_GetRegSessionRank(uint32 chid);
		int send_2833_flag = 0;
		int bypass_2833_chid_limit = 0;
		static int dtmf_removal_pre[MAX_VOIP_CH_NUM] = {0};
		uint32 isConference;
		uint32 *pSessRank = NULL;
		...
		pSessRank = chanInfo_GetRegSessionRank(chid);
		...
		for (j=0; j < SessNum; j++)
		{
			...
			if (rfc2833_payload_type_local[pSessRank[j]]!=0 && rfc2833_payload_type_remote[pSessRank[j]]!=0)//Not DTMF Inband
			{
				send_2833_flag = 1;
			}
			else
			...
			if ((dtmf_mode[chid] != 1)/*SIP INFO*/&& (send_2833_flag == 1))
			{
				...
				if (send_dtmf_flag[pSessRank[j]] == 1 && (i%PCM_PERIOD) == 0)
				{
...
label_send_2833_now:
					...
					if ((dtmf_removal[chid][0] == 1) && (dtmf_removal_pre[chid] == 0))
						rfc2833_count_add(pSessRank[j], 3); // add 30ms 
