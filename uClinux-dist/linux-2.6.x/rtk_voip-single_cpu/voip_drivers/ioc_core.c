...
static struct timer_list leds_blinking_timer;
...
static void leds_blinking_timer_func( unsigned long data )
{
...
{
	init_timer( &leds_blinking_timer ); /* init once */
	
	leds_blinking_timer.expires = jiffies + LED_BLINKING_PERIOD;
	leds_blinking_timer.function = leds_blinking_timer_func;
