...
#ifdef SI32267
	if ((hProslic)->deviceId->chipType >= SI3226 && (hProslic)->deviceId->chipType <= SI3227)
		return Si3226_InitBroadcast_GUI(hProslic);
...
int ProSLIC_SetMuteStatus (proslicChanType_ptr hProslic, ProslicMuteModes muteEn){
	...
	printk("<<<<<<<<< need implement ProSLIC_SetMuteStatus for Si3217x >>>>>>>>>>\n");
#endif
	return 1;
}
...
int ProSLIC_SetLoopbackMode (proslicChanType_ptr hProslic, ProslicLoopbackModes newMode){
	...
	printk("<<<<<<<<< need implement ProSLIC_SetLoopbackMode for Si3217x >>>>>>>>>>\n");
#endif
	return 1;
}
...
{
#ifdef SI3217X
	if (hProslic->deviceId->chipType >= SI32177 && hProslic->deviceId->chipType <= SI32178)
	{
		SI3217X_Set_SO_DTx_Loopback(hProslic, enable);
	}
#else
