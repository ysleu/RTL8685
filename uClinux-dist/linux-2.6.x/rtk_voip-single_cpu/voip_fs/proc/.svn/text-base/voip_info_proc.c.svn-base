...
#ifndef CONFIG_AUDIOCODES_VOIP
#if !defined(CONFIG_RTK_VOIP_ETHERNET_DSP_IS_HOST) && defined(CONFIG_RTK_VOIP_G729AB)
		extern char* g729_version_str;
		n += sprintf(buf + n, "%s", g729_version_str);
#endif
#endif
	}
...
void __init voip_proc_add_echo_init(void)
{
	struct proc_dir_entry *voip_add_echo_proc;

	add_delayed_echo=0;

	voip_add_echo_proc = create_proc_entry(PROC_VOIP_DIR "/add_echo", 0644, 
	                                                            NULL);
	if (voip_add_echo_proc == NULL) {
		printk("voip_add_echo_proc NULL!! \n;");
		return;
	}
	voip_add_echo_proc->read_proc = voip_add_echo_read;
	voip_add_echo_proc->write_proc = voip_add_echo_write;
}

void __exit voip_proc_add_echo_exit( void )
{
	remove_proc_entry( PROC_VOIP_DIR "/add_echo", NULL );
}
...
void __init voip_proc_echo_sel_init(void)
{
	...
	voip_echo_sel_proc = create_proc_entry(PROC_VOIP_DIR "/ec_select", 
	                                              0644, NULL);
	if (voip_echo_sel_proc == NULL) {
		printk("voip_echo_sel_proc NULL!! \n");
		return;
	}
	voip_echo_sel_proc->read_proc = voip_echo_sel_read;
	voip_echo_sel_proc->write_proc = voip_echo_sel_write;
}

void __exit voip_proc_echo_sel_exit( void )
{
	remove_proc_entry( PROC_VOIP_DIR "/ec_select", NULL );
}
