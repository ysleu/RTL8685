...
struct RTK_TRAP_profile Rtp_send[MAX_VOIP_CH_NUM][FIFO_NUM];
char Rtp_fifo[MAX_VOIP_CH_NUM][FIFO_NUM*512];
static unsigned long length = sizeof(struct RTK_TRAP_profile);
static unsigned int pload_len = 0;
static unsigned char rtp_w[MAX_VOIP_CH_NUM] = {0}, rtp_r[MAX_VOIP_CH_NUM] = {0};
static unsigned char pload_w[MAX_VOIP_CH_NUM] ={0}, pload_r[MAX_VOIP_CH_NUM] = {0};

#endif

static void send(struct RTK_TRAP_profile *ptr, void *ptr_data, uint32 data_len)
{
	uint8 *tmp;
	uint32 tos;
	Tsudphdr *udphdr;
